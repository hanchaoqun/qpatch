...
void qpatch_status_error(int act, int status)
{
      ...
      if(act == 3){
            LOG(LOG_ERR, "PatchState:(%s)  ExpectState:(INIT or LOADED or ACTIVED)!",  state_str);
      }
      if(act == 2){
            LOG(LOG_ERR, "PatchState:(%s)  ExpectState:(LOADED or ACTIVED)!",  state_str);
      }
      if(act == 1){
...
int qpatch_dsp_patch(pid_t pid, const char *objname, const char *dllname, int symelang)
{
    int rc = 0;
    ...
    do{
        ...
        callret = 0;
        if((rc = ptrace_pp_inject_library(pp, dllname, "qpatch_check", paradata, paradatalen, &dllhandle, &callret)) < 0){
        ...
        paradatalen = sizeof(qci);
        callret = 0;
        if((rc = ptrace_pp_call_library(pp, dllhandle, "qpatch_open_room", paradata, paradatalen, &callret, 1)) < 0){
            LOG(LOG_ERR, "Error to inject library!");
            rc = -1;
            break;
        }
        ...
        {
            LOG(LOG_ERR, "Error to call qpatch_open_room() ret %d!", qpatch_open_room_ret);
            rc = -1;
            break;
        }
        ...
        if((rc = ptrace_pp_read_data(pp, (uintptr_t)qpatch_open_room_ret, (unsigned char *)&mroom, sizeof(struct qpatch_mmap_room))) < 0){
            LOG(LOG_ERR, "Read mmap room error!");
            rc = -1;
            break;
        }
        if(mroom.mhdr.version != QPATCH_VERSION){
            LOG(LOG_ERR, "Read mmap room version(%u) is not expect(%u)!", mroom.mhdr.version, QPATCH_VERSION);
            rc = -1;
            break;
        }
        ...
        if(mroom.mhdr.status == QPATCH_STATUS_INIT){
            ...
            LOG(LOG_INFO, "Read mmap room status(%u) is not expect!", mroom.mhdr.status);
        ...
        if(baseptr != (void *)((size_t)qpatch_open_room_ret + (size_t)LNK_OBJ_BASE_OFFSET_IN_ROOM)){
            LOG(LOG_ERR, "Qpatch room error ptr2data(baseptr), addr(%p) + baseoff(%p) != baseptr(%p)!!", qpatch_open_room_ret, LNK_OBJ_BASE_OFFSET_IN_ROOM, baseptr);
            rc = -1;
            break;
        }
        base_rephdr_ptr = (void *)mroom.ptr2rephdr;
        if(base_rephdr_ptr != (void *)((size_t)qpatch_open_room_ret + (size_t)LNK_REPHDR_OFFSET_IN_ROOM)){
            LOG(LOG_ERR, "Qpatch room error base_rephdr_ptr, addr(%p) + rephdroff(%p) != rephdrptr(%p)!!", qpatch_open_room_ret, LNK_REPHDR_OFFSET_IN_ROOM, base_rephdr_ptr);
            rc = -1;
            break;
        }
        repfuns_num = mroom.rephdr.repfuns_num;
        if(repfuns_num<0 || repfuns_num>LNK_MAX_REP_FUNC_COUNT){
            LOG(LOG_ERR, "Qpatch room error repfuns_num(%u) MAX(%u)!!", repfuns_num, LNK_MAX_REP_FUNC_COUNT);
            rc = -1;
            break;
        }
        hokfuns_num = mroom.rephdr.hookfuns_num;
        if(hokfuns_num<0 || hokfuns_num>LNK_MAX_HOOK_FUNC_COUNT){
            LOG(LOG_ERR, "Qpatch room error hokfuns_num(%u) MAX(%u)!!", hokfuns_num, LNK_MAX_HOOK_FUNC_COUNT);
            rc = -1;
            break;
        }
        ...
        for(fidx = 0; fidx < repfuns_num; fidx++){
            ...
            LOG(LOG_INFO, "Actived(%s) repfun<%s> oldaddr(%p) newaddr(%p) oldsize(%u) newsize(%u) ", 
                    (repf->isreplaced)?"YES":"NO", repf->name, repf->oldaddr, repf->newaddr, repf->oldsize, repf->newsize);
        }
        if(rc < 0){
            break;
        }
        ...
        for(hidx = 0; hidx < hokfuns_num; hidx++){
            ...
            LOG(LOG_INFO, "Actived(%s) hookfun<%s> oldaddr(%p) newaddr(%p) oldsize(%u) newsize(%u) ", 
                    (hokf->isreplaced)?"YES":"NO", hokf->oldname, hokf->oldaddr, hokf->newaddr, hokf->oldsize, hokf->newsize);
        }
        if(rc < 0){
            break;
        }
        ...
        LOG(LOG_DEBUG, "Qpatch room addr(%p) len(%u) baseoff(%p) baseptr(%p) rephdroff(%p) rephdrptr(%p) repfuns(%u) hookfuns(%u)",
                       qpatch_open_room_ret, mroom_total_len, 
                       LNK_OBJ_BASE_OFFSET_IN_ROOM, baseptr, 
                       LNK_REPHDR_OFFSET_IN_ROOM, base_rephdr_ptr, 
...
int qpatch_rol_patch(pid_t pid, const char *objname, const char *dllname, int symelang)
{
    int rc = 0;
        struct linkable_elf_internals *li = NULL;
    struct ptrace_pid * pp = NULL;
    ...
    do{
        ...
        if(mroom.mhdr.status == QPATCH_STATUS_INIT){
           ...
           do{
                ...
                paradatalen = sizeof(qci);
                callret = 0;
                if((rc = ptrace_pp_call_library(pp, dllhandle, "qpatch_close_room", paradata, paradatalen, &callret, 1)) < 0){
                    LOG(LOG_ERR, "Error to inject library!");
                    rc = -1;
                    break;
                }
                ...
                {
                    LOG(LOG_ERR, "Error to call qpatch_close_room() ret %d!", qpatch_close_room_ret);
                    rc = -1;
                    break;
                }
        ...
        if(mroom.mhdr.status != QPATCH_STATUS_LOADED && mroom.mhdr.status != QPATCH_STATUS_ACTIVED){
            LOG(LOG_INFO, "Read mmap room status(%u) is not expect(QPATCH_STATUS_LOADED:%u or QPATCH_STATUS_ACTIVED:%u)!", 
                mroom.mhdr.status, QPATCH_STATUS_LOADED, QPATCH_STATUS_ACTIVED);
            qpatch_status_error(2, mroom.mhdr.status);
            rc = -1;
            break;
        }
        LOG(LOG_INFO, "Read mmap room len(%u)  calc mroom_total_len(%u)!", mroom.mhdr.roomlen, mroom_total_len);
        mroom_total_len = mroom.mhdr.roomlen;
        /*
        if(mroom.roomlen != mroom_total_len){
            LOG(LOG_ERR, "Read mmap room len(%u) is not equal(%u), will force rmv patch!", mroom.roomlen, mroom_total_len);
            rc = -1;
            break;
        }*/
        baseptr = (void *)mroom.ptr2data;
        ...
        for(fidx = 0; fidx < repfuns_num; fidx++){
            repf = &(mroom.rephdr.repfuns[fidx]);
            if(!repf->isreplaced){
                LOG(LOG_DEBUG, "Act-Fun<%s> is not replaced(%u), ignore.", repf->name, repf->isreplaced);
                rel_repfuns_num++;
                continue;
            }
            if(repf->oldsize <= LNK_MAX_CODE_BAK_LEN){
                LOG(LOG_DEBUG, "Act-Fun<%s> oldsize(%u) need at least large(%u), ignore.", repf->name, repf->oldsize, LNK_MAX_CODE_BAK_LEN);
                continue;
            }
            if(repf->funbaklen != LNK_MAX_CODE_BAK_LEN){
                LOG(LOG_DEBUG, "Act-Fun<%s> funbaklen(%u) not equal to (%u), ignore.", repf->name, repf->funbaklen, LNK_MAX_CODE_BAK_LEN);
                continue;
            }
            memset(jumpcode, 0, LNK_MAX_CODE_BAK_LEN);
                if ((rc = ptrace_pid_readarray(pp->hp->pid, repf->oldaddr, jumpcode, LNK_MAX_CODE_BAK_LEN)) < 0){
                LOG(LOG_ERR, "Act-Fun<%s> can't read fun from(%p) to(%p) len(%u), ignore.", repf->name, repf->oldaddr, jumpcode, LNK_MAX_CODE_BAK_LEN);
                        break;
                }
				
#if __WORDSIZE != 64
            offset = repf->newaddr - repf->oldaddr - JMP_OPER_CODELEN;
            LOG(LOG_DEBUG, "Act-Fun<%s> replacing from(%p)[%02x%02x%02x%02x%02x%02x%02x%02x] to(%p)[%02x%02x%02x%02x%02x%02x%02x%02x] offset(%p)...", 
                repf->name, repf->newaddr,
                jumpcode[0], jumpcode[1], jumpcode[2], jumpcode[3], jumpcode[4], jumpcode[5], jumpcode[6], jumpcode[7],
                repf->oldaddr,
                repf->funbak[0], repf->funbak[1], repf->funbak[2], repf->funbak[3], repf->funbak[4], repf->funbak[5], repf->funbak[6], repf->funbak[7],
                offset);
            if(jumpcode[0] != JMP_OPER_CODE){
                LOG(LOG_DEBUG, "Act-Fun<%s> first opercode(%02x) is not(%02x), ignore.", repf->name, jumpcode[0], JMP_OPER_CODE);
                continue;
            }
            if(offset != *((long *)(&jumpcode[1])) ){
                LOG(LOG_DEBUG, "Act-Fun<%s> offset(%p) is not(%p), ignore.", repf->name, *((long *)(&jumpcode[1])), offset);
                continue;
            }
#else
            ...
            if(memcmp(jumpcode, g_pMac, N_OFFSET) != 0){
                LOG(LOG_DEBUG, "Act-Fun<%s> first opercode(%02x) is not(%02x), ignore.", repf->name, jumpcode[0], 0xff);
                continue;
            }
            if(repf->newaddr != *((long *)(&jumpcode[N_OFFSET])) ){
                LOG(LOG_DEBUG, "Act-Fun<%s> offset(%p) is not(%p), ignore.", repf->name, *((long *)(&jumpcode[N_OFFSET])), repf->newaddr);
                continue;
            }
#endif

            if ((rc = ptrace_pid_writearray(pp->hp->pid, repf->oldaddr, repf->funbak, LNK_MAX_CODE_BAK_LEN)) < 0){
                LOG(LOG_ERR, "Act-Fun<%s> can't write fun from(%p) to(%p) len(%u), ignore.", repf->name, repf->funbak, repf->oldaddr, LNK_MAX_CODE_BAK_LEN);
                        break;
            }
            repf->funbaklen = 0;
            ...
            rel_repfuns_num++;
            LOG(LOG_INFO, "Act-Fun<%s> replace from(%p) to(%p) ok.", repf->name, repf->oldaddr, repf->newaddr);
        }
        if(rc < 0){
            break;
        }
        ...
        for(hidx = 0; hidx < hokfuns_num; hidx++){
            hokf = &(mroom.rephdr.hookfuns[hidx]);
            if(!hokf->isreplaced){
                LOG(LOG_DEBUG, "Act-Fun-Hook<%s> is not replaced(%u), ignore.", hokf->oldname, hokf->isreplaced);
                rel_hokfuns_num++;
                continue;
            }
            if(hokf->oldsize <= LNK_MAX_CODE_BAK_LEN){
                LOG(LOG_DEBUG, "Act-Fun-Hook<%s> oldsize(%u) need at least large(%u), ignore.", hokf->oldname, hokf->oldsize, LNK_MAX_CODE_BAK_LEN);
                continue;
            }
            if(hokf->funbaklen != LNK_MAX_CODE_BAK_LEN){
                LOG(LOG_DEBUG, "Act-Fun-Hook<%s> funbaklen(%u) not equal to (%u), ignore.", hokf->oldname, hokf->funbaklen, LNK_MAX_CODE_BAK_LEN);
                continue;
            }
            memset(jumpcode, 0, LNK_MAX_CODE_BAK_LEN);
            if ((rc = ptrace_pid_readarray(pp->hp->pid, hokf->oldaddr, jumpcode, LNK_MAX_CODE_BAK_LEN)) < 0){
                LOG(LOG_ERR, "Act-Fun-Hook<%s> can't read fun from(%p) to(%p) len(%u), ignore.", hokf->oldname, hokf->oldaddr, jumpcode, LNK_MAX_CODE_BAK_LEN);
                        break;
            }
			
#if __WORDSIZE != 64
            offset = hokf->newaddr - hokf->oldaddr - JMP_OPER_CODELEN;
            LOG(LOG_DEBUG, "Act-Fun-Hook<%s> replacing from(%p)[%02x%02x%02x%02x%02x%02x%02x%02x] to(%p)[%02x%02x%02x%02x%02x%02x%02x%02x] offset(%p)...", 
                hokf->oldname, hokf->newaddr,
                jumpcode[0], jumpcode[1], jumpcode[2], jumpcode[3], jumpcode[4], jumpcode[5], jumpcode[6], jumpcode[7],
                hokf->oldaddr,
                hokf->funbak[0], hokf->funbak[1], hokf->funbak[2], hokf->funbak[3], hokf->funbak[4], hokf->funbak[5], hokf->funbak[6], hokf->funbak[7],
                offset);
            if(jumpcode[0] != JMP_OPER_CODE){
                LOG(LOG_DEBUG, "Act-Fun-Hook<%s> first opercode(%02x) is not(%02x), ignore.", hokf->oldname, jumpcode[0], JMP_OPER_CODE);
                continue;
            }
            if(offset != *((long *)(&jumpcode[1])) ){
                LOG(LOG_DEBUG, "Act-Fun-Hook<%s> offset(%p) is not(%p), ignore.", hokf->oldname, *((long *)(&jumpcode[1])), offset);
                continue;
            }
#else
            const char *g_pMac = "\xff\x25\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0";
            ...
            if ((rc = ptrace_pid_writearray(pp->hp->pid, hokf->oldaddr, hokf->funbak, LNK_MAX_CODE_BAK_LEN)) < 0){
                LOG(LOG_ERR, "Act-Fun-Hook<%s> can't write fun from(%p) to(%p) len(%u), ignore.", hokf->oldname, hokf->funbak, hokf->oldaddr, LNK_MAX_CODE_BAK_LEN);
                        break;
            }
            hokf->funbaklen = 0;
            ...
            rel_hokfuns_num++;
            LOG(LOG_INFO, "Act-Fun-Hook<%s> replace from(%p) to(%p) ok.", hokf->oldname, hokf->oldaddr, hokf->newaddr);
        }
        if(rc < 0){
            break;
        }
        ...
        if((rc = ptrace_pid_writearray(pp->hp->pid, (uintptr_t)qpatch_open_room_ret, (unsigned char *)&mroom, sizeof(struct qpatch_mmap_room))) < 0){
       ...
       do{
            ...
            paradatalen = sizeof(qci);
            callret = 0;
            if((rc = ptrace_pp_call_library(pp, dllhandle, "qpatch_close_room", paradata, paradatalen, &callret, 0)) < 0){
                LOG(LOG_ERR, "Error to inject library!");
                rc = -1;
                break;
            }
        ...
        LOG(LOG_INFO, "Qpatch room addr(%p) len(%u) baseoff(%p) baseptr(%p) rephdroff(%p) rephdrptr(%p) repfuns(%u) hookfuns(%u)",
                       qpatch_open_room_ret, mroom_total_len, 
                       LNK_OBJ_BASE_OFFSET_IN_ROOM, baseptr, 
                       LNK_REPHDR_OFFSET_IN_ROOM, base_rephdr_ptr, 
                       mroom.rephdr.repfuns_num, mroom.rephdr.hookfuns_num);
        LOG(LOG_INFO, "Rol patch is ok status change to QPATCH_STATUS_INIT(%u).", QPATCH_STATUS_INIT);
    }while(0);
    ...
    if(rc < 0){
       ...
       do{
            ...
            paradatalen = sizeof(qci);
            callret = 0;
            if((rc = ptrace_pp_call_library(pp, dllhandle, "qpatch_close_room", paradata, paradatalen, &callret)) < 0){
                LOG(LOG_ERR, "Error to inject library!");
                rc = -1;
                break;
            }
...
int qpatch_act_patch(pid_t pid, const char *objname, const char *dllname, int symelang)
{
    int rc = 0;
        struct linkable_elf_internals *li = NULL;
    struct ptrace_pid * pp = NULL;
    ...
    do{
        ...
        if(mroom.mhdr.status != QPATCH_STATUS_LOADED){
            LOG(LOG_INFO, "Read mmap room status(%u) is not expect(QPATCH_STATUS_LOADED:%u)!", mroom.mhdr.status, QPATCH_STATUS_LOADED);
            qpatch_status_error(1, mroom.mhdr.status);
            rc = -1;
            break;
        }
        if(mroom.mhdr.roomlen != mroom_total_len){
            LOG(LOG_ERR, "Read mmap room len(%u) is not equal(%u), please rol patch and retry!", mroom.mhdr.roomlen, mroom_total_len);
            rc = -1;
            break;
        }
        baseptr = (void *)mroom.ptr2data;
        ...
        for(hidx = 0; hidx < hokfuns_num; hidx++){
            hokf = &(mroom.rephdr.hookfuns[hidx]);
            if(hokf->isreplaced){
                LOG(LOG_INFO, "Act-Fun-Hook<%s> isreplaced(%u).", hokf->oldname, hokf->isreplaced);
                rel_hokfuns_num++;
                continue;
            }
            if(hokf->oldsize <= LNK_MAX_CODE_BAK_LEN){
                LOG(LOG_INFO, "Act-Fun-Hook<%s> oldsize(%u) need at least large(%u).", hokf->oldname, hokf->oldsize, LNK_MAX_CODE_BAK_LEN);
                continue;
            }

			if ((rc = ptrace_pid_readarray(pp->hp->pid, hokf->oldaddr, hokf->funbak, LNK_MAX_CODE_BAK_LEN)) < 0){
                LOG(LOG_ERR, "Act-Fun-Hook<%s> can't read fun from(%p) to(%p) len(%u).", hokf->oldname, hokf->oldaddr, hokf->funbak, LNK_MAX_CODE_BAK_LEN);
                        break;
            }
            searchsize = (hokf->oldsize < LNK_MAX_CODE_ORIG_FUNHEAD_SEARCH_LEN) ? hokf->oldsize : LNK_MAX_CODE_ORIG_FUNHEAD_SEARCH_LEN;

			if ((rc = ptrace_pid_readarray(pp->hp->pid, hokf->oldaddr, tmpopcode, searchsize)) < 0){
                LOG(LOG_ERR, "Act-Fun-Hook<%s> can't read fun from(%p) to(%p) len(%u).", hokf->oldname, hokf->oldaddr, tmpopcode, searchsize);
                        break;
            }
            ...
            if(origheadersize > LNK_MAX_CODE_ORIG_FUNHEAD_LEN){
                LOG(LOG_INFO, "Act-Fun-Hook<%s> oldsize(%u) passheadersize(%u) exceed(LNK_MAX_CODE_ORIG_FUNHEAD_LEN:%u).", 
                    hokf->oldname, hokf->oldsize, origheadersize,LNK_MAX_CODE_ORIG_FUNHEAD_LEN);
                continue;
            }

            memset(hokf->origfunhead, NOP_OPER_CODE, LNK_MAX_CODE_ORIG_FUNHEAD_LEN);
            memcpy(hokf->origfunhead, tmpopcode, origheadersize);
            memset(hokf->jmporigfuntail, NOP_OPER_CODE, LNK_MAX_CODE_JMP_ORIG_FUNTAIL_LEN);
            ...
            memcpy(&hokf->jmporigfuntail[N_OFFSET], (unsigned char*) & (offset), sizeof(long));
#else
            hokf->jmporigfuntail[0] = JMP_OPER_CODE;
            size_t origfun_entry = LNK_HOOK_FUN_ORIGFUNTAIL_ENTRY((size_t)base_rephdr_ptr, hidx);
            offset = (hokf->oldaddr + origheadersize) - origfun_entry - JMP_OPER_CODELEN;
            memcpy(&hokf->jmporigfuntail[1], (unsigned char*) & (offset), sizeof(long));
#endif
            ...
            offset = hokf->newaddr - hokf->oldaddr - JMP_OPER_CODELEN;
            LOG(LOG_ERR, "origfun_entry:%d,newaddr:%p,offset:%d",origfun_entry,hokf->newaddr,offset);
            ...
            */
            ...
            LOG(LOG_INFO, "Act-Fun-Hook replacing from<%s>(%p)[%02x%02x%02x%02x%02x%02x%02x%02x] to<%s>(%p)[%02x%02x%02x%02x%02x%02x%02x%02x] offset(%p)...",
                hokf->oldname, hokf->oldaddr,
                hokf->funbak[0], hokf->funbak[1], hokf->funbak[2], hokf->funbak[3], hokf->funbak[4], hokf->funbak[5], hokf->funbak[6], hokf->funbak[7],
                hokf->newname, hokf->newaddr,
                jumpcode[0], jumpcode[1], jumpcode[2], jumpcode[3], jumpcode[4], jumpcode[5], jumpcode[6], jumpcode[7],
                offset);
#endif
            ...
            hokf->isreplaced = TRUE;
                if ((rc = ptrace_pid_writearray(pp->hp->pid, hokf->oldaddr, (unsigned char *)jumpcode, LNK_MAX_CODE_BAK_LEN)) < 0){
                LOG(LOG_ERR, "Act-Fun-Hook<%s> can't write fun from(%p) to(%p) len(%u).", hokf->oldname, jumpcode, hokf->oldaddr, LNK_MAX_CODE_BAK_LEN);
                        break;
                }
            LOG(LOG_INFO, "Act-Fun-Hook replace from<%s>(%p) to<%s>(%p) ok.", hokf->oldname, hokf->oldaddr, hokf->newname, hokf->newaddr);
            rel_hokfuns_num++;
        }
        if(rc < 0){
            break;
        ...
        for(fidx = 0; fidx < repfuns_num; fidx++){
            repf = &(mroom.rephdr.repfuns[fidx]);
            if(repf->isreplaced){
                LOG(LOG_INFO, "Act-Fun<%s> isreplaced(%u).", repf->name, repf->isreplaced);
                rel_repfuns_num++;
                continue;
            }
            if(repf->oldsize <= LNK_MAX_CODE_BAK_LEN){
                LOG(LOG_INFO, "Act-Fun<%s> oldsize(%u) need at least large(%u).", repf->name, repf->oldsize, LNK_MAX_CODE_BAK_LEN);
                continue;
            }
                if ((rc = ptrace_pid_readarray(pp->hp->pid, repf->oldaddr, repf->funbak, LNK_MAX_CODE_BAK_LEN)) < 0){
                LOG(LOG_ERR, "Act-Fun<%s> can't read fun from(%p) to(%p) len(%u).", repf->name, repf->oldaddr, repf->funbak, LNK_MAX_CODE_BAK_LEN);
                        break;
                }
#if __WORDSIZE == 64
            /*
            ...
            */
            ...
            repf->isreplaced = TRUE;
                if ((rc = ptrace_pid_writearray(pp->hp->pid, repf->oldaddr, (unsigned char *)jumpcode, LNK_MAX_CODE_BAK_LEN)) < 0){
                LOG(LOG_ERR, "Act-Fun<%s> can't write fun from(%p) to(%p) len(%u).", repf->name, jumpcode, repf->oldaddr, LNK_MAX_CODE_BAK_LEN);
                        break;
                }
            LOG(LOG_INFO, "Act-Fun<%s> replace from(%p) to(%p) ok.", repf->name, repf->oldaddr, repf->newaddr);
            rel_repfuns_num++;
        ...
        LOG(LOG_INFO, "Qpatch room addr(%p) len(%u) baseoff(%p) baseptr(%p) rephdroff(%p) rephdrptr(%p) repfuns(%u) hookfuns(%u)",
                       ...
                       LNK_REPHDR_OFFSET_IN_ROOM, base_rephdr_ptr, 
                       mroom.rephdr.repfuns_num, mroom.rephdr.hookfuns_num);
        LOG(LOG_INFO, "Act patch is ok status change to QPATCH_STATUS_ACTIVED(%u).", QPATCH_STATUS_ACTIVED);
    }while(0);
    if(rc < 0){
        LOG(LOG_DEBUG, "Detaching from PID %d.", pp->hp->pid);
        if (ptrace_pid_detach(pp->hp->pid) < 0) {
...
int qpatch_lod_patch(pid_t pid, const char *objname, const char *dllname, int symelang, const char *pat_symbol)
{
    int rc = 0;
    struct linkable_elf_internals *li = NULL;
    struct ptrace_pid * pp = NULL;
    ...
    do{
        ...
        if((rc = ptrace_pp_read_data(pp, (uintptr_t)qpatch_open_room_ret, (unsigned char *)&mhdr, sizeof(struct qpatch_mmap_room_hdr))) < 0){
            LOG(LOG_ERR, "Read mmap room header error!");
            rc = -1;
            break;
        }
        if(mhdr.version != QPATCH_VERSION){
            LOG(LOG_ERR, "Read mmap room version(%u) is not expect(%u)!", mhdr.version, QPATCH_VERSION);
            rc = -1;
            break;
        }
        if(mhdr.status != QPATCH_STATUS_INIT){
            LOG(LOG_ERR, "Read mmap room status(%u) is not expect(QPATCH_STATUS_INIT:%u)!", mhdr.status, QPATCH_STATUS_INIT);
            qpatch_status_error(0, mhdr.status);
            rc = -1;
            break;
        }
        if(mhdr.roomlen != 0 && mhdr.roomlen != mroom_total_len){
            LOG(LOG_ERR, "Read mmap room len(%u) is not equal(%u), please rol patch and retry!", mhdr.roomlen, mroom_total_len);
            rc = -1;
            break;
        }
        ...
        LOG(LOG_INFO, "Qpatch room addr(%p) baseoff(%p) baseptr(%p) rephdroff(%p) rephdrprt(%p) bssptr(%p) pltgotptr(%p) objsize(%d)", 
            qpatch_open_room_ret, LNK_OBJ_BASE_OFFSET_IN_ROOM, baseptr, LNK_REPHDR_OFFSET_IN_ROOM, base_rephdr_ptr, bssptr, pltgotptr, objsize);
        li = (struct linkable_elf_internals *)linkable_elf_obj_create(pid, symelang, objname, (void *)baseptr, (void *)base_rephdr_ptr, bssptr, LNK_MAX_BSS_LEN, pltgotptr, LNK_MAX_PLTGOT_LEN, pat_symbol);
        if( !li ){
        ...
        if(li->objlen != objsize) {
            LOG(LOG_ERR, "Error to match the obj_image<%d> size to obj_file<%d> !", li->objlen, objsize);
            rc = -1;
            break;
        }

        /* li->objptr has addtional bss len: li->objlen + LNK_MAX_PLTGOT_LEN + LNK_MAX_BSS_LEN */
        if((rc = ptrace_pp_write_data(pp, (uintptr_t)baseptr, (unsigned char *)li->objptr, li->objlen+LNK_MAX_PLTGOT_LEN+LNK_MAX_BSS_LEN)) < 0){
        ...
        /*
        if((rc = ptrace_pp_write_zero_data(pp, (uintptr_t)bssptr, LNK_MAX_BSS_LEN)) < 0){
            LOG(LOG_ERR, "Qpatch load patch write zero to bssptr error!");
            rc = -1;
            break;
        }
        */
        ...
        if((rc = ptrace_pp_write_data(pp, (uintptr_t)qpatch_open_room_ret, (unsigned char *)&mroom, sizeof(struct qpatch_mmap_room))) < 0){
            LOG(LOG_ERR, "Qpatch load patch write room header to dest error!");
            rc = -1;
            break;
        }
        LOG(LOG_INFO, "Qpatch room addr(%p) len(%u) baseoff(%p) baseptr(%p) rephdroff(%p) rephdrptr(%p) repfuns(%u) hookfuns(%u)",
                       ...
                       LNK_REPHDR_OFFSET_IN_ROOM, base_rephdr_ptr, 
                       mroom.rephdr.repfuns_num, mroom.rephdr.hookfuns_num);
        LOG(LOG_INFO, "Lod patch is ok status change to QPATCH_STATUS_LOADED(%u).", QPATCH_STATUS_LOADED);
    }while(0);
...
int main(int argc, char *argv[])
{
    ...
    if(action == 0) {
        rc = qpatch_lod_patch((pid_t)pid, objname, tmpdllfullname, symelang, pat_symbol_file);
    } else if(action == 1) {
