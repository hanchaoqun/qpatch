...
enum symbol_elf_bit { ELF_IS_NEITHER, ELF_IS_32BIT, ELF_IS_64BIT };
...
struct symbol_elf_internals {
  ...
  off_t sechdr_offset; /* Object file must have section-header-table, EXE file
                          has. */
  void* sechdrs;       /* section headers, sechdrs[0..sechdr_num] */
  size_t sechdr_num;
  size_t sechdr_size; /* total buffer size, sechdr_num*sizeof(*sechdrs[0]) */
  size_t
      secnametbl_idx; /* Object file must have section-name-strings-table-sechr,
                         strsectblhdr = &sechdrs[ei->secnametbl_idx]; strsectbl
                         = malloc(strsectblhdr->sh_size) */
  char* strsectbl;    /* string table for section names,
                         strsectbl[sechdrs[0].sh_name] */
...
enum { ELF_LIB_LD = 0, ELF_LIB_C, ELF_LIB_DL, ELF_LIB_PTHREAD, ELF_LIB_MAX };

enum symbol_elf_elang { ELF_E_LANG_C = 0, ELF_E_LANG_GO, ELF_E_LANG_MAX };
...
struct symbol_elf_pid {
  ...
  struct symbol_elf_interp
      exe_interp; /* dynamic loader from .interp in the exe */
...
extern struct symbol_elf_pid* symbol_pid_create_inner(pid_t pid, int symelang,
                                                      int needlibc);
extern void symbol_pid_destroy(struct symbol_elf_pid* hp);
extern uintptr_t symbol_pid_find_func(struct symbol_elf_pid* hp,
                                      const char* symbol, size_t* sz);
extern uintptr_t symbol_pid_find_global(struct symbol_elf_pid* hp,
                                        const char* symbol, size_t* sz);
extern uintptr_t symbol_ld_find_func(struct symbol_elf_pid* hp,
                                     const char* symbol, size_t* sz);
extern uintptr_t symbol_ld_find_global(struct symbol_elf_pid* hp,
                                       const char* symbol, size_t* sz);
...
extern void symbol_elf_ei_destory(struct symbol_elf_internals* ei,
                                  int needfreesymbol);
extern void* symbol_elf_load_section_tables(struct symbol_elf_internals* ei,
                                            Elf_Shdr* sechdr, size_t* outnum);
extern void* symbol_elf_load_section_strings(struct symbol_elf_internals* ei,
                                             Elf_Shdr* strh,
                                             size_t* outtotalsize);
