...
int ptrace_pid_attach(pid_t pid)
{
    ...
    if (result == -1L) {
        int err = errno;
        LOG(LOG_ERR, "Ptrace Attach for PID %d failed with error: %s", pid, strerror(err));
        return -1;
    }
    return 0;
}
...
int ptrace_pid_detach(pid_t pid)
{
        if (ptrace(PTRACE_DETACH, pid, NULL, NULL) < 0) {
                int err = errno;
                LOG(LOG_DEBUG, "Ptrace Detach for PID %d failed with error: %s", pid, strerror(err));
                return -1;
        }
        return 0;
}
...
int ptrace_pid_cont(pid_t pid)
{
        if (ptrace(PTRACE_CONT, pid, NULL, NULL) < 0) {
                int err = errno;
                LOG(LOG_ERR, "Ptrace Continue for PID %d failed with error: %s", pid, strerror(err));
                return -1;
        }
        return 0;
}
...
int ptrace_pid_syscall(pid_t pid)
{
    if (ptrace(PTRACE_SYSCALL, pid, NULL, NULL) < 0) {
        int err = errno;
        LOG(LOG_ERR, "Ptrace Syscall for PID %d failed with error: %s", pid, strerror(err));
        return -1;
    }
    return 0;
}
...
int ptrace_pid_set_watchthread(pid_t pid)
{
    ...
    if (ptrace(PTRACE_SETOPTIONS, pid, NULL, PTRACE_O_TRACECLONE | PTRACE_O_TRACEFORK | PTRACE_O_TRACEVFORK) < 0) {
        int err = errno;
        LOG(LOG_ERR, "Ptrace setoptions for PID %d failed with error: %s", pid, strerror(err));
        return -1;
    }
    return 0;
}
...
int ptrace_pid_wait_thread(pid_t pid, pid_t *childpid, pid_t *newpid)
{
    ...
    *newpid = 0;
    *childpid = 0;
    // TODO: change to use : ptrace(PTRACE_GETSIGINFO, 25618, NULL, {si_signo=SIGTRAP, si_code=SI_KERNEL, si_value={int=-1447215360, ptr=0x7ffda9bd3f00}}) = 0
    while (1) {
        new_pid = 0;
        status = 0;
        ...
        *childpid = child_pid;
        if (child_pid < 0) {
            int err = errno;
            LOG(LOG_ERR, "Waitpid for PID %d failed with error: %s", pid, strerror(err));
            return -1;
        }
        ...
        if (WIFSTOPPED(status)) {
            ...
            if (signum == SIGTRAP) {
                ...
                LOG(LOG_INFO, "SIG(%d) resent to TID %d PID %d.", signum, child_pid, pid);
        ...
        while (1) {
            ...
            if (WIFSTOPPED(status)) {
                ...
                if (signum == SIGTRAP || signum == SIGSTOP || signum == SIGSEGV) {
                    ...
                    LOG(LOG_INFO, "PID %d was not traced by me, signum(%d) not SIGTRAP(%d) or SIGSTOP(%d), continue waiting...", pid, signum, SIGTRAP, SIGSTOP);
...
int ptrace_pid_getregs(pid_t pid, struct user *regs)
{
        ...
        if (ptrace(PTRACE_GETREGS, pid, NULL, regs) < 0) {
                int err = errno;
                LOG(LOG_ERR, "Ptrace Getregs for PID %d failed with error: %s", pid, strerror(err));
                return -1;
        }
        return 0;
}
...
int ptrace_pid_setregs(pid_t pid, const struct user *regs)
{
        ...
        if (ptrace(PTRACE_SETREGS, pid, NULL, regs) < 0) {
                int err = errno;
                LOG(LOG_ERR, "Ptrace Setregs for PID %d failed with error: %s", pid, strerror(err));
                return -1;
        }
        return 0;
}

int ptrace_pid_readarray(pid_t pid, uintptr_t target,
                                          unsigned char *data, size_t datasz)
{
        ...
        while (pos < datasz) {
            ...
            err = errno;
            if (err != 0) {
                LOG(LOG_ERR, "Ptrace ReadArray for PID %d failed with error: %s", pid, strerror(err));
                return -1;
            }
            idx += sizeof(size_t);
...
int ptrace_pid_writezero(pid_t pid, uintptr_t target, size_t datasz)
{
    ...
    if ( sizeof(size_t) != sizeof(long) ) {
        LOG(LOG_ERR, "sizeof: size_t %d, long %d, not equal", sizeof(size_t), sizeof(long) );
        return -1;
    }
    ...
    for ( idx = 0; idx < (datasz / word_size); idx++ ) {
        if (ptrace(PTRACE_POKEDATA, pid, target + (idx * word_size), pokedata) < 0) {
            int err = errno;
    ...
    if ( spare_len > 0 ) {
        ...
        err = errno;
        if (err != 0) {
            LOG(LOG_ERR, "Ptrace ReadInt for PID %d failed with error: %s", pid, strerror(err));
            return -1;
        }
...
int ptrace_pid_writelong(pid_t pid, uintptr_t target, uintptr_t invalue)
{
        ...
        if (ptrace(PTRACE_POKEDATA, pid, target, (void *)invalue) < 0) {
                LOG(LOG_ERR, "Ptrace WriteInt for PID %d failed with error: %s", pid, strerror(err));
                return -1;
        }
        return 0;
}

int ptrace_pid_call_func_noparam(pid_t pid, uintptr_t funcaddr, uintptr_t *outcallret)
{
    int rc = 0;
    ...
    do{
        ...
        for (idx = 0; idx < sizeof(stack)/sizeof(uintptr_t); ++idx) {
            if ((rc = ptrace_pid_readlong(pid, PTRACE_REG_SP(iregs) +
            idx * sizeof(size_t), &stack[idx])) < 0)
                break;
            LOG(LOG_INFO, "CopyFrom idx[%u] SP[%p] V[%p].", idx, PTRACE_REG_SP(iregs) + idx * sizeof(size_t), stack[idx]);
        }
        if (rc < 0) {
        ...
        for (idx = 0; idx < sizeof(stack)/sizeof(uintptr_t); ++idx) {
            if ((rc = ptrace_pid_writelong(pid, PTRACE_REG_SP(oregs) +
            idx * sizeof(size_t), stack[idx])) < 0)
                break;
            LOG(LOG_INFO, "CopyBack idx[%u] SP[%p] V[%p].", idx, PTRACE_REG_SP(oregs) + idx * sizeof(size_t), stack[idx]);
        }
        if (rc < 0)
        {
...
int ptrace_pp_read_data(struct ptrace_pid *pp, uintptr_t target, unsigned char *outdata, size_t outdatalen)
{
    int rc = 0;
    ...
    do {
        ...
        if (ptrace_pid_readarray(pp->hp->pid, target, outdata, outdatalen) < 0) {
            rc = -1;
            LOG(LOG_ERR, "PID %d read array from(%p) to(%p) size(%u) error : %s", pp->hp->pid, target, outdata, outdatalen, strerror(errno));
            break;
        }
        if (ptrace_pid_detach(pp->hp->pid) < 0) {
            rc = -1;
            LOG(LOG_ERR, "PID %d will be unstable, continue error : %s", pp->hp->pid, strerror(errno));
            break;
        }
...
int ptrace_pp_write_zero_data(struct ptrace_pid *pp, uintptr_t target, size_t indatalen)
{
    int rc = 0;
    ...
    do {
        ...
        if (ptrace_pid_writezero(pp->hp->pid, target, indatalen) < 0) {
            rc = -1;
            LOG(LOG_ERR, "PID %d write zero array to(%p)  size(%u) error : %s", pp->hp->pid, target, indatalen, strerror(errno));
            break;
        }
        if (ptrace_pid_detach(pp->hp->pid) < 0) {
            rc = -1;
...
int ptrace_pp_write_data(struct ptrace_pid *pp, uintptr_t target, const unsigned char *indata, size_t indatalen)
{
    int rc = 0;
        ...
        do {
            ...
            if (ptrace_pid_writearray(pp->hp->pid, target, indata, indatalen) < 0)
                    {
                rc = -1;
                LOG(LOG_ERR, "PID %d write array to(%p) from(%p) size(%u) error : %s", pp->hp->pid, target, indata, indatalen, strerror(errno));
                            break;
                    }
            if (ptrace_pid_detach(pp->hp->pid) < 0)
                    {
...
int ptrace_pp_call_library(struct ptrace_pid *pp, uintptr_t indlladdr, const char *callsymbol,
                                                        const unsigned char *paradata, size_t paradatalen, uintptr_t *outcallret, int needattach)
{
        ...
        do {
                ...
                if ((rc = ptrace_pid_getregs(pp->hp->pid, &oregs)) < 0)
                        break;
                memcpy(&iregs, &oregs, sizeof(oregs));
                LOG(LOG_DEBUG, "Copying stack out...");
                for (idx = 0; idx < sizeof(stack)/sizeof(uintptr_t); ++idx) {
                        if ((rc = ptrace_pid_readlong(pp->hp->pid, PTRACE_REG_SP(iregs) +
                                                        idx * sizeof(size_t), &stack[idx])) < 0)
                                break;
						LOG(LOG_DEBUG, "CopyFrom idx[%u] SP[%p] V[%p].", idx, PTRACE_REG_SP(iregs) + idx * sizeof(size_t), stack[idx]);
                }
                if (rc < 0)
                {
					LOG(LOG_ERR, "Copy stack error %s.", strerror(errno));
                ...
                if (callsymbol && pp->hp->fn_dlsym && indlladdr != 0) {
					...
					PTRACE_ASM_SET_BREAKPOINT(pp->hp->pid, iregs, rc);
					PTRACE_CHECK_RC_AND_BREAK(rc,"SET_BREAKPOINT");
					PTRACE_ASM_PASS_ARGS2FUNC(pp->hp->pid, iregs, pp->hp->fn_dlsym, indlladdr, (heapptr), rc);
					PTRACE_CHECK_RC_AND_BREAK(rc,"PASS_ARGS2FUNC");
					...
					if (result != 0) {
						...
						LOG(LOG_DEBUG, "Return value from call %s(): %p", callsymbol, (void *)result);
						if (outcallret)
							*outcallret = result;
					} 
					else 
					{
						LOG(LOG_ERR, "Unable to find %s(). Dll might already have been injected earlier.", callsymbol);
						if (outcallret)
							*outcallret = 0;
					}
                } 
    ...
    do{
        ...
        if ((rc = ptrace_pid_setregs(pp->hp->pid, &oregs)) < 0) {
                LOG(LOG_ERR, "PID %d will be unstable, set original registers error : %s", pp->hp->pid, strerror(errno));
                break;
        }
        LOG(LOG_DEBUG, "Copying stack back...");
        for (idx = 0; idx < sizeof(stack)/sizeof(uintptr_t); ++idx) {
                if ((rc = ptrace_pid_writelong(pp->hp->pid, PTRACE_REG_SP(oregs) +
                                                idx * sizeof(size_t), stack[idx])) < 0)
                        break;
                LOG(LOG_DEBUG, "CopyBack idx[%u] SP[%p] V[%p].", idx, PTRACE_REG_SP(oregs) + idx * sizeof(size_t), stack[idx]);
        }
        if (rc < 0)
        {
                LOG(LOG_ERR, "Copy stack back error %s.", strerror(errno));
...
int ptrace_pid_inject_libc(pid_t pid, int elang, const char *libcname, const size_t libcsize)
{
    int rc = 0;
    int idx = 0;
    /* The stack is read-write and not executable */
    ...
    do {
        ...
        LOG(LOG_DEBUG, "Copying stack out...");
        for (idx = 0; idx < sizeof(stack)/sizeof(uintptr_t); ++idx) {
            if ((rc = ptrace_pid_readlong(pid, PTRACE_REG_SP(iregs) + idx * sizeof(size_t), &stack[idx])) < 0)
                break;
            LOG(LOG_DEBUG, "CopyFrom idx[%u] SP[%p] V[%p].", idx, PTRACE_REG_SP(iregs) + idx * sizeof(size_t), stack[idx]);
        }
        if (rc < 0)
        ...
        PTRACE_ASM_MEMCPY2STACK(pid, iregs, libcname, strlen(libcname) + 1, inj_libcname, rc);
        PTRACE_CHECK_RC_AND_BREAK(rc,"MEMCPY2STACK");
        PTRACE_ASM_SET_BREAKPOINT(pid, iregs, rc);
        PTRACE_CHECK_RC_AND_BREAK(rc,"SET_BREAKPOINT");
        PTRACE_ASM_PASS_ARGS2SYSCALL6(pid, iregs, P_SYS_openat, (int32_t)P_AT_FDCWD, inj_libcname, P_O_RDONLY, 0, 0, 0, rc);
        ...
        // syscall soptr = mmap(0, inj_libcsize, PROT_EXEC | PROT_READ, MAP_PRIVATE, inj_fd, 0);
        PTRACE_ASM_SET_BREAKPOINT(pid, iregs, rc);
        PTRACE_CHECK_RC_AND_BREAK(rc,"SET_BREAKPOINT");
        PTRACE_ASM_PASS_ARGS2SYSCALL7(pid, iregs, P_SYS_mmap, 0, inj_libcsize, (P_PROT_EXEC | P_PROT_READ | P_PROT_WRITE), P_MAP_PRIVATE, inj_fd, 0, 0, rc);
        PTRACE_CHECK_RC_AND_BREAK(rc,"PASS_ARGS2SYSCALL");
    ...
    do{
        ...
        if (ptrace_pid_detach(pid) < 0) {
            rc = -1;
            LOG(LOG_ERR, "PID %d will be unstable, continue error : %s", pid, strerror(errno));
            break;
        }
...
int ptrace_pp_inject_library(struct ptrace_pid *pp, const char *dll, const char *callsymbol,
                                                        ...
                                                        uintptr_t *outdlladdr, uintptr_t *outcallret)
{
    ...
    if (!dll || !pp || !pp->hp) {
        LOG(LOG_ERR, "Invalid arguments dll<%p> pp<%p> pp->hp<%p>.", dll, pp, pp->hp);
        return -1;
    }
    ...
    do {
        ...
        LOG(LOG_DEBUG, "Copying stack out...");
        for (idx = 0; idx < sizeof(stack)/sizeof(uintptr_t); ++idx) {
            if ((rc = ptrace_pid_readlong(pp->hp->pid, PTRACE_REG_SP(iregs) + idx * sizeof(size_t), &stack[idx])) < 0)
                break;
            LOG(LOG_DEBUG, "CopyFrom idx[%u] SP[%p] V[%p].", idx, PTRACE_REG_SP(iregs) + idx * sizeof(size_t), stack[idx]);
        }
        if (rc < 0)
        ...
        PTRACE_ASM_SET_BREAKPOINT(pp->hp->pid, iregs, rc);
        PTRACE_CHECK_RC_AND_BREAK(rc,"SET_BREAKPOINT");
        PTRACE_ASM_PASS_ARGS2FUNC(pp->hp->pid, iregs, pp->hp->fn_dlopen, heapptr, (RTLD_NOW | RTLD_LOCAL), rc);
        PTRACE_CHECK_RC_AND_BREAK(rc,"PASS_ARGS2FUNC");
        ...
        if (callsymbol && pp->hp->fn_dlsym && *outdlladdr != 0) {
            if (0 /* TODO: not work for now */ ) {
                // Force loading the .so file into memory (sometime it does not auto load)
                if (ptrace_pid_readarray(pp->hp->pid, *outdlladdr, (unsigned char *)&dll_link_map, dll_link_map_len) == 0) {
                    if (ptrace_pid_readarray(pp->hp->pid, (uintptr_t)dll_link_map.l_name, (unsigned char *)dll_link_map_file, dllsz) == 0) {
                        LOG(LOG_DEBUG, "Read dll_link_map.l_name(%p) = %s", (uintptr_t)dll_link_map.l_name, dll_link_map_file);
                    }
                }
            }
            LOG(LOG_DEBUG, "Start call function [dlsym]...");
            PTRACE_ASM_SET_BREAKPOINT(pp->hp->pid, iregs, rc);
            PTRACE_CHECK_RC_AND_BREAK(rc,"SET_BREAKPOINT");
            PTRACE_ASM_PASS_ARGS2FUNC(pp->hp->pid, iregs, pp->hp->fn_dlsym, *outdlladdr, (heapptr + dllsz), rc);
            PTRACE_CHECK_RC_AND_BREAK(rc,"PASS_ARGS2FUNC");
            PTRACE_ASM_SET_REGS(pp->hp->pid, "dlsym", iregs, rc);
            PTRACE_ASM_CALL_FUNC(pp->hp->pid, "dlsym", iregs, rc);
            PTRACE_CHECK_RC_AND_BREAK(rc,"CALL_FUNC [dlsym]");
            ...
            if (callsymbol) {
                ...
                LOG(LOG_DEBUG, "No symbol was specified. _init() might have been invoked.");
            }
            if (outcallret)
                *outcallret = 0;
        }
