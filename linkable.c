...
static Elf_Addr linkable_elf_pat_repable(const char *symbolname,
                                         const char *pat_symbol_file,
                                         size_t *outsymbolsize, int type);
...
static void *linkable_file_read_to_buf(const char *filename, size_t *outlen,
                                       long addlen) {
  void *baseptr;
...
static int linkable_elf_is_object_type(struct linkable_elf_internals *li) {
  if (!li || li->ei.sechdr_offset == 0 || li->ei.sechdr_size == 0) return -1;

  if (li->ei.type != ET_REL) {
    LOG(LOG_ERR, "Object file type error %d.(relocatable file expected)",
        li->ei.type);
    return -1;
  }
  ...
  if (li->ei.sechdrs == NULL || li->ei.strsectbl == NULL) {
    LOG(LOG_ERR,
        "Object file type error (%p or %p).(section-header-table or "
        "section-name-string-table missing)",
        li->ei.sechdrs, li->ei.strsectbl);
    return -1;
  }

  if (!li->ei.sechdr_idx_bss && !li->ei.sechdr_idx_text &&
      !li->ei.sechdr_idx_data && !li->ei.sechdr_idx_rodata) {
    LOG(LOG_ERR,
        "Object file type error missing sections bss_idx(%u) text_idx(%u) "
        "data_idx(%u) rodata_idx(%u) ",
        li->ei.sechdr_idx_bss, li->ei.sechdr_idx_text, li->ei.sechdr_idx_data,
        li->ei.sechdr_idx_rodata);
    return -1;
  }
...
static Elf_Addr linkable_elf_rsv_symbol_repable(
    struct linkable_elf_internals *li, const char *symbolname,
    size_t *outsymbolsize, int st) {
  Elf_Addr tptr = 0;
  int inexe = TRUE;
  size_t symbolsize = 0;
  ...
  do {
    if (!tptr) {
      if (LNK_SH_TYPE_EITHER == st || LNK_SH_TYPE_FUN == st) {
        if ((tptr = symbol_pid_find_func(li->hp, symbolname, &symbolsize)) >
            0) {
          inexe = TRUE;
          break;
        }
      ...
      if (LNK_SH_TYPE_EITHER == st || LNK_SH_TYPE_OBJ == st) {
        if ((tptr = symbol_pid_find_global(li->hp, symbolname, &symbolsize)) >
            0) {
          inexe = TRUE;
          break;
        }
    ...
    if (!tptr) {
      if (LNK_SH_TYPE_EITHER == st || LNK_SH_TYPE_FUN == st) {
        if ((tptr = symbol_ld_find_func_repable(li->hp, symbolname,
                                                &symbolsize)) > 0) {
          inexe = FALSE;
          break;
        }
      ...
      if (LNK_SH_TYPE_EITHER == st || LNK_SH_TYPE_OBJ == st) {
        if ((tptr = symbol_ld_find_global_repable(li->hp, symbolname,
                                                  &symbolsize)) > 0) {
          inexe = FALSE;
          break;
        }
  ...
  LOG(LOG_DEBUG, "Symbol<%s> Addr<%p> Size<%u> Exe<%u> resolved!", symbolname,
      tptr, symbolsize, inexe);
  return tptr;
}

static Elf_Addr linkable_elf_rsv_symbol(struct linkable_elf_internals *li,
  ...
  do {
    ...
    if (!tptr) {
      ...
      if (LNK_SH_TYPE_EITHER == st || LNK_SH_TYPE_OBJ == st) {
        if ((tptr = symbol_ld_find_global(li->hp, symbolname, &symbolsize)) >
            0) {
          inexe = FALSE;
          break;
        }
...
/*
static void linkable_elf_rsv_upd_sym_addr(struct linkable_elf_internals *li,
Elf_Sym *sym, uintptr_t newaddr)
{
    size_t idx = 0;
    Elf_Sym  *orsym     = NULL;
    ...
    for(idx = 0; idx < li->ei.symbols_num; idx++)
    {
        ...
        if(orsym->st_name == sym->st_name){
            LOG(LOG_INFO, "Symbol<%s> Addr<%p> update to NewAddr<%p>!",
symbol->name, symbol->address, newaddr); symbol->address = (uintptr_t)newaddr;
        }
    }
}
*/

static struct linkable_elf_hook_fun *linkable_elf_find_hookfun(
    struct linkable_elf_internals *li, const char *oldsymbolname) {
...
static int linkable_elf_rsv_rela(struct linkable_elf_internals *li,
  ...
  if (!li || !rela_info || !li->ei.sechdrs || !li->ei.sechdr_num) return -1;

  /*Only two types of relocation are found in a relocatable object file. So its
    enough if we handle these two*/

  /* if(rela_info->sechdr_dst_idx > li->ei.sechdr_num){
      LOG(LOG_ERR, "Relocation dest section index is out of range %u/%u.",
  rela_info->sechdr_dst_idx, li->ei.sechdr_num); return -1;
  } */
  if (rela_info->sechdr_idx > li->ei.sechdr_num) {
    LOG(LOG_ERR, "Relocation self section index is out of range %u/%u.",
        rela_info->sechdr_idx, li->ei.sechdr_num);
    return -1;
  }
  if (rela_info->sechdr_sym_idx > li->ei.sechdr_num) {
    LOG(LOG_ERR, "Relocation symb section index is out of range %u/%u.",
        rela_info->sechdr_sym_idx, li->ei.sechdr_num);
    return -1;
  }

  if (rela_info->sechdr_str_idx > li->ei.sechdr_num) {
    LOG(LOG_ERR, "Relocation symb-str section index is out of range %u/%u.",
        rela_info->sechdr_str_idx, li->ei.sechdr_num);
    return -1;
  }
  ...
  {
      LOG(LOG_INFO, "Relo-dest-section unsupport: idx<%d> not in .bss<%d>
  .text<%d> .data<%d> .rodata<%d>", sechdr_rela->sh_info, li->ei.sechdr_idx_bss,
  li->ei.sechdr_idx_text, li->ei.sechdr_idx_data, li->ei.sechdr_idx_rodata);
      return 0;
  }*/
  if (sechdr_rela->sh_info >= li->ei.sechdr_num) {
    LOG(LOG_DEBUG,
        "Relo-dest-section unsupport: idx<%d> not in .bss<%d> .text<%d> "
        ".data<%d> .rodata<%d>",
        sechdr_rela->sh_info, li->ei.sechdr_idx_bss, li->ei.sechdr_idx_text,
        li->ei.sechdr_idx_data, li->ei.sechdr_idx_rodata);
    return 0;
  }
  ...
  base_rel_hdr_addr =
      (void *)((Elf_Addr)inruntimebase + (Elf_Addr)sechdr_dest_offset);
  from_rel_hdr_addr =
      (void *)((Elf_Addr)li->objptr + (Elf_Addr)sechdr_dest_offset);
  ...
  if (name) {
    LOG(LOG_DEBUG,
        "Relocating rt_base:%p from_obj:%p secname:%s sec_off:%p "
        "rt_hdr_addr:%p from_hdr_addr:%p",
        inruntimebase, li->objptr, name, sechdr_dest_offset, base_rel_hdr_addr,
        from_rel_hdr_addr);
  } else {
    LOG(LOG_DEBUG,
        "Relocating rt_base:%p from_obj:%p secname:%s sec_off:%p "
        "rt_hdr_addr:%p from_hdr_addr:%p",
        inruntimebase, li->objptr, "N/A", sechdr_dest_offset, base_rel_hdr_addr,
        from_rel_hdr_addr);
  }

  /* read rela section tables */
  relas = (Elf_Rela *)symbol_elf_load_section_tables(&(li->ei), sechdr_rela,
                                                     &relas_num);
  if (!relas) {
    return -1;
  }
  ...
  r_syms = (Elf_Sym *)symbol_elf_load_section_tables(&(li->ei), sechdr_symb,
                                                     &r_syms_num);
  if (!r_syms) {
    free(relas);
    relas = NULL;
  ...
  strsymtbl = (char *)symbol_elf_load_section_strings(&(li->ei), sechdr_str,
                                                      &strsymtbl_size);
  if (!strsymtbl) {
    free(relas);
    relas = NULL;
  ...
  LOG(LOG_DEBUG, "Relo-sechdr size<%d>, entsize<%d>, offset<%d>",
      sechdr_rela->sh_size, sechdr_rela->sh_entsize, sechdr_rela->sh_offset);
  for (relaidx = 0; relaidx < (sechdr_rela->sh_size / sechdr_rela->sh_entsize);
       relaidx++) {
    rela = (Elf_Rela *)((unsigned long)relas +
                        (relaidx * sechdr_rela->sh_entsize));
    base_rel_itm_addr =
        (void *)((Elf_Addr)rela->r_offset + (Elf_Addr)base_rel_hdr_addr);
    from_rel_itm_addr =
        (void *)((Elf_Addr)rela->r_offset + (Elf_Addr)from_rel_hdr_addr);
    ...
    if( SHN_COMMON == r_sym->st_shndx) {
        LOG(LOG_ERR, "Relo-symbol symbol<%s> is in SHN_COMMON<%u>, i haven't
    find out how to solve it, so don't support!", name2, SHN_COMMON);
        LOG(LOG_ERR, "Relo-symbol symbol<%s> , please INIT IT WITH NONE ZERO
    VALUE!", name2); sym_un_res_cnt++; continue; }else if( li->ei.sechdr_idx_bss
    == r_sym->st_shndx) { LOG(LOG_ERR, "Relo-symbol symbol<%s> is in .bss<%u>, i
    haven't find out how to solve it, so don't support!", name2,
    li->ei.sechdr_idx_bss); LOG(LOG_ERR, "Relo-symbol symbol<%s> , please INIT
    IT WITH NONE ZERO VALUE!", name2); sym_un_res_cnt++; continue;
    }*/
    /* if( SHN_COMMON == r_sym->st_shndx ||  li->ei.sechdr_idx_bss ==
     * r_sym->st_shndx) */
    if (SHN_COMMON == r_sym->st_shndx ||
        li->ei.sechdrs_types[r_sym->st_shndx] == ELF_SECT_BSS) {
      if (r_sym->st_size <= 0) {
        LOG(LOG_INFO,
            "BSS-Relo-symbol symbol<%s> is in BSS<%u>, and size<%d> is less or "
            "equal zero, not support, ignore!",
            name2, r_sym->st_shndx, r_sym->st_size);
        // sym_un_res_cnt++;
        continue;
      }
      ...
      if (relaSymAddrs[r_symbidx] != 0) {
        ...
        LOG(LOG_DEBUG,
            "BSS-Relo-idx<%d>tp<%d>smbidx<%d>smbshidx<%d>oldsmb<%s>smbvl<%p> "
            "t_base<%p>rtsymadr<%p>rela<%p> symsize<%d> inbssoffsetbase<%p>",
            relaidx, r_reltype, r_symbidx, r_sym->st_shndx, name2,
            r_sym->st_value, inruntimebase, base_symbol_addr, rela,
            r_sym->st_size, inbssoffsetbase);
      } else {
        /* allocate memory for new find bss value */
        base_symbol_addr = (Elf_Addr)inbssoffsetbase;
        relaSymAddrs[r_symbidx] = base_symbol_addr;
        inbssoffsetbase =
            (void *)((Elf_Addr)inbssoffsetbase + (Elf_Addr)(r_sym->st_size));
        LOG(LOG_DEBUG,
            "BSS-Relo-idx<%d>tp<%d>smbidx<%d>smbshidx<%d>newsmb<%s>smbvl<%p> "
            "t_base<%p>rtsymadr<%p>rela<%p> symsize<%d> inbssoffsetbase<%p>",
            relaidx, r_reltype, r_symbidx, r_sym->st_shndx, name2,
            r_sym->st_value, inruntimebase, base_symbol_addr, rela,
            r_sym->st_size, inbssoffsetbase);
        if (inbssoffsetbase > inbssoffsetend) {
          LOG(LOG_ERR,
              "BSS-Relo-symbol symbol<%s> is in BSS<%u>, and size<%d> exceed "
              "limit <%d>!",
              name2, r_sym->st_shndx, r_sym->st_size, li->bsslen);
          sym_un_res_cnt++;
          continue;
        }
      }
      ...
      base_symbol_addr = (Elf_Addr)inruntimebase + (Elf_Addr)r_sym->st_value +
                         (Elf_Addr)sh_offset;
      LOG(LOG_DEBUG,
          "Relo-idx<%d>tp<%d>smbidx<%d>smbshidx<%d>smb<%s>smbvl<%p>smbhdroff<%"
          "p>rt_base<%p>rtsymadr<%p>rela<%p>",
          relaidx, r_reltype, r_symbidx, r_sym->st_shndx, name2,
          r_sym->st_value, sh_offset, inruntimebase, base_symbol_addr, rela);
    }

    /* undefine in obj file, find from outside */
    if (r_sym->st_shndx == SHN_UNDEF) {
      if (r_sym->st_name <= 0) {
        LOG(LOG_ERR, "Relo-symbol symbol<%s> unsupport: symbol name idx<%d>",
            name2, r_sym->st_name);
        sym_un_res_cnt++;
        continue;
      }
      if ((STB_GLOBAL != ELF_ST_BIND(r_sym->st_info)) &&
        ...
        LOG(LOG_ERR,
            "Relo-symbol symbol<%s> unsupport: BIND<%u> not STB_GLOBAL<%d> "
            "STB_WEAK<%d>",
            name2, ELF_ST_BIND(r_sym->st_info), STB_GLOBAL, STB_WEAK);
        sym_un_res_cnt++;
        continue;
      }
      if ((ELF_ST_TYPE(r_sym->st_info) == STT_SECTION) ||
        ...
        LOG(LOG_ERR,
            "Relo-symbol symbol<%s> has been handled before, TYPE<%u> is "
            "STT_SECTION<%d> STT_FILE<%d>",
            name2, ELF_ST_TYPE(r_sym->st_info), STT_SECTION, STT_FILE);
        relacnt++;
        continue;
      }
      ...
      if ((base_symbol_addr = linkable_elf_rsv_symbol(
               li, name2, &symbolsize, shtype, pat_symbol)) <= 0) {
        LOG(LOG_ERR,
            "Relo-symbol-external symbol<%s> doesn't find, Un-Resolved!",
            name2);
        sym_un_res_cnt++;
        continue;
      } else {
        LOG(LOG_DEBUG,
            "Relo-symbol-external sym<%s> rt_sym_addr:S<%p> "
            "rt_rel_itm_addr:P<%p> from_rel_itm_addr:A<%p>",
            name2, base_symbol_addr, base_rel_itm_addr, from_rel_itm_addr);
      }
      if (base_symbol_addr <= 0) {
        LOG(LOG_ERR,
            "Relo-symbol-external symbol<%s> need retry, but we don't support!",
            name2);
        sym_un_res_cnt++;
        continue;
      }
      ...
      if (base_symbol_addr <= 0) {
        LOG(LOG_ERR,
            "Relo-symbol-internal symbol<%s> need retry, but we don't support!",
            name2);
        sym_un_res_cnt++;
        continue;
      }
      LOG(LOG_DEBUG,
          "Relo-symbol-internal sym<%s> rt_sym_addr:S<%p> "
          "rt_rel_itm_addr:P<%p> from_rel_itm_addr:A<%p>",
          name2, base_symbol_addr, base_rel_itm_addr, from_rel_itm_addr);
    }
    /* NONEED!! linkable_elf_rsv_upd_sym_addr(li, r_sym,
     * (uintptr_t)base_symbol_addr); */
    hf = (struct linkable_elf_hook_fun *)linkable_elf_find_hookfun(li, name2);
    if (hf != NULL) {
      /* This function need be hooked, so we redirect the inner call jump to
       * "origfunhead", set base_symbol_addr->"origfunhead"  */
      base_symbol_addr = LNK_HOOK_FUN_ORIGFUNHEAD_ENTRY(
          (Elf_Addr)li->base_rephdr_ptr, hf->idx);
      LOG(LOG_DEBUG,
          "Relo-symbol-hooked sym<%s> base_rephdr_ptr:<%p> "
          "CHANGED_rt_sym_addr:S<%p> rt_rel_itm_addr:P<%p> "
          "from_rel_itm_addr:A<%p>",
          name2, li->base_rephdr_ptr, base_symbol_addr, base_rel_itm_addr,
          from_rel_itm_addr);
    }
    switch (r_reltype) {
#if __WORDSIZE == 64
      case R_X86_64_GOTPCREL:  // word32 0x9  (G+GOT) + A - P
        ...
        if (inruntimebase && li->objptr) {
          ...
          if (relaSymAddrsPlt[r_symbidx] != 0) {
            ...
            LOG(LOG_DEBUG,
                "GOT-Relo-idx<%d>tp<%d>smbidx<%d>smbshidx<%d>oldsmb<%s>smbvl<%"
                "p> t_base<%p>rtsymadr<%p>rela<%p> symsize<%d> "
                "inpltgotoffsetbase<%p>",
                relaidx, r_reltype, r_symbidx, r_sym->st_shndx, name2,
                r_sym->st_value, inruntimebase, base_symbol_addr, rela,
                r_sym->st_size, inpltgotoffsetbase);
          } else {
            /* allocate memory for new find GOT value */
            got_g_symbol_item =
                (struct linkable_elf_pltgot_item
                     *)((char *)li->objptr +
                        (long)LNK_PLTGOT_BASE_OFFSET_IN_OBJ(li->objlen) +
                        (long)inpltgotoffsetbase - (long)li->pltgotptr);
            ...
            got_g_symbol_addr = (Elf_Addr)inpltgotoffsetbase +
                                (Elf_Addr)LNK_ELF_PLTGOT_ADDRLONG_OFFSET;
            relaSymAddrsPlt[r_symbidx] = got_g_symbol_addr;
            inpltgotoffsetbase =
                (void *)((Elf_Addr)inpltgotoffsetbase +
                         (Elf_Addr)(sizeof(struct linkable_elf_pltgot_item)));
            LOG(LOG_DEBUG,
                "GOT-Relo-idx<%d>tp<%d>smbidx<%d>smbshidx<%d>newsmb<%s>smbvl<%"
                "p> t_base<%p>rtsymadr<%p>rela<%p> symsize<%d> "
                "inpltgotoffsetbase<%p>",
                relaidx, r_reltype, r_symbidx, r_sym->st_shndx, name2,
                r_sym->st_value, inruntimebase, base_symbol_addr, rela,
            ...
            if (inpltgotoffsetbase > inpltgotoffsetend) {
              LOG(LOG_ERR,
                  "GOT-Relo-symbol symbol<%s> is in GOT<%u>, value<%p> exceed "
                  "limit <%p>!",
                  name2, r_sym->st_shndx, inpltgotoffsetbase,
                  inpltgotoffsetend);
              sym_un_res_cnt++;
              break;
            }
        ...
        relacnt++;
        LOG(LOG_DEBUG,
            "Relo-type R_X86_64_GOTPCREL:<%d>, smb<%s> G+GOT:%p S(sAd):%p P:%p "
            "A(*P):%p <(G+GOT)+A-P>(N*P):%p",
      ...
      case R_X86_64_PLT32:  // word32 0x9  L + A - P
        ...
        if (inruntimebase && li->objptr) {
          ...
          if (relaSymAddrsPlt[r_symbidx] != 0) {
            got_g_symbol_addr = relaSymAddrsPlt[r_symbidx];
            LOG(LOG_DEBUG,
                "PLT-Relo-idx<%d>tp<%d>smbidx<%d>smbshidx<%d>oldsmb<%s>smbvl<%"
                "p> t_base<%p>rtsymadr<%p>rela<%p> symsize<%d> "
                "inpltgotoffsetbase<%p>",
                relaidx, r_reltype, r_symbidx, r_sym->st_shndx, name2,
                r_sym->st_value, inruntimebase, base_symbol_addr, rela,
            ...
            inpltgotoffsetbase =
                (void *)((Elf_Addr)inpltgotoffsetbase +
                         (Elf_Addr)(sizeof(struct linkable_elf_pltgot_item)));
            LOG(LOG_DEBUG,
                "PLT-Relo-idx<%d>tp<%d>smbidx<%d>smbshidx<%d>newsmb<%s>smbvl<%"
                "p> t_base<%p>rtsymadr<%p>rela<%p> symsize<%d> "
                "inpltgotoffsetbase<%p>",
                relaidx, r_reltype, r_symbidx, r_sym->st_shndx, name2,
                r_sym->st_value, inruntimebase, base_symbol_addr, rela,
            ...
            if (inpltgotoffsetbase > inpltgotoffsetend) {
              LOG(LOG_ERR,
                  "PLT-Relo-symbol symbol<%s> is in GOT<%u>, value<%p> exceed "
                  "limit <%p>!",
                  name2, r_sym->st_shndx, inpltgotoffsetbase,
                  inpltgotoffsetend);
              sym_un_res_cnt++;
        ...
        relacnt++;
        LOG(LOG_DEBUG,
            "Relo-type R_X86_64_PLT32:<%d>, smb<%s> G+GOT:%p S(sAd):%p P:%p "
            "A(*P):%p <L+A-P>(N*P):%p",
            r_reltype, name2, got_g_symbol_addr, S, P, A, RV);
        break;
      ...
      case R_X86_64_64:  // word64 same with R_386_32 = 0x1
        ...
        relacnt++;
        LOG(LOG_DEBUG,
            "Relo-type R_X86_64_64:<%d>, smb<%s> S(sAd):%p P:%p A(*P):%p "
            "<S+A>(N*P):%p",
      ...
      case R_X86_64_32:  // word32 0xa
        ...
        relacnt++;
        LOG(LOG_DEBUG,
            "Relo-type R_X86_64_32:<%d>, smb<%s> S(sAd):%p P:%p A(*P):%p "
            "<S+A>(N*P):%p",
            r_reltype, name2, S, P, A, RV);
        break;
#else
      case R_386_32: /* word32 *P = S + *P */ /* S + A */
        ...
        relacnt++;
        LOG(LOG_DEBUG,
            "Relo-type R_386_32:<%d>, smb<%s> S(sAd):%p P:%p A(*P):%p "
            "<S+A>(N*P):%p",
            r_reltype, name2, S, P, A, RV);
        break;
#endif
      ...
      case R_386_PC32: /* word32 *P = S + *P - P */ /* S + A - P */
        ...
        if (inruntimebase && li->objptr) {
          if (sechdr_rela->sh_entsize == sizeof(Elf_Rela)) {
            A = rela->r_addend;
            LOG(LOG_DEBUG,
                "Relo-type R_386_PC32:<%d>, smb<%s> r_addend<%p> A<%p>",
                r_reltype, name2, rela->r_addend, A);
          } else {
            A = *(Elf_Addr *)P_FROM;
            LOG(LOG_DEBUG,
                "Relo-type R_386_PC32:<%d>, smb<%s> P_FROM<%p> A<%p>",
                r_reltype, name2, P_FROM, A);
          }
          RV = S + A - P;
          /* *(Elf_Addr *)P = RV; */
          *(Elf32_Addr *)P_FROM = (Elf32_Addr)RV;
        ...
        relacnt++;
        LOG(LOG_DEBUG,
            "Relo-type R_386_PC32:<%d>, smb<%s> S(sAd):%p P:%p A(*P):%p "
            "<S+A-P>(N*P):%p",
            r_reltype, name2, S, P, A, RV);
        break;
#if 0
  ...
  if (relacnt == 0 || sym_un_res_cnt != 0) {
    LOG(LOG_ERR,
        "Relocate total count zero (%u) or has (%u) Un-Resolved symbols!!",
        relacnt, sym_un_res_cnt);
    return -1;
  }
...
static int linkable_elf_is_has_innersymbol(struct linkable_elf_internals *li,
                                           char *name) {
  int find = FALSE;
  size_t idx = 0;
  Elf_Sym *sym = NULL;
...
static int linkable_elf_build_hookfuns(struct linkable_elf_internals *li,
                                       const char *pat_symbol) {
  int rc = 0;
  size_t idx = 0;
  Elf_Sym *sym = NULL;
  ...
  for (idx = 0; idx < li->ei.symbols_num; idx++) {
    ...
    sym = &(symbol->sym);
    LOG(LOG_DEBUG,
        "Hook-Info-Find smb<%s> add<%p> size<%u> shndx<%u> type<%u> bind<%u>.",
        symbol->name, symbol->address, symbol->size, sym->st_shndx,
    ...
    if (sym->st_shndx >= li->ei.sechdr_num ||
        li->ei.sechdrs_types[sym->st_shndx] != ELF_SECT_TEXT) {
      LOG(LOG_DEBUG, "Rep-Fun-Ignore smb<%s> shndx<%u> != .text<%u>",
          symbol->name, sym->st_shndx, li->ei.sechdr_idx_text);
      continue;
    }
    ...
    if (!sym->st_size ||
        !((Elf_Addr)sym->st_value + (Elf_Addr)sym_hdr->sh_offset) ||
        !sym->st_name) {
      LOG(LOG_DEBUG,
          "Hook-Info-Ignore smb<%s> nameidx<%u> value<%p> hdroff<%p> size<%u>, "
          "one of them is NULL.",
          symbol->name, sym->st_name, sym->st_value, sym_hdr->sh_offset,
          sym->st_size);
      continue;
    }
    if (strlen(symbol->name) <= strlen(LNK_HOOK_FUN_NAME_PREFIX)) {
      LOG(LOG_DEBUG,
          "Hook-Info-Ignore smb<%s> doesn't match prefix "
          "<LNK_HOOK_FUN_NAME_PREFIX:%s>.",
    ...
    LOG(LOG_INFO,
        "Hook-Info-Find smb<%s> orgsmb<%s> match prefix "
        "<LNK_HOOK_FUN_NAME_PREFIX:%s>.",
        symbol->name, old_symbol_name, LNK_HOOK_FUN_NAME_PREFIX);
    if (TRUE == linkable_elf_is_has_innersymbol(li, old_symbol_name)) {
      LOG(LOG_INFO,
          "Hook-Info-Ignore smb<%s> is find inner obj-file, that's doesn't "
          "support!",
          old_symbol_name);
      continue;
    }
    shtype = LNK_SH_TYPE_FUN;
    if ((old_symbol_addr = linkable_elf_rsv_symbol_repable(
             li, old_symbol_name, &old_symbol_size, shtype)) <= 0) {
      if ((old_symbol_addr = linkable_elf_pat_repable(
               old_symbol_name, pat_symbol, &old_symbol_size, 1)) <= 0) {
        LOG(LOG_INFO, "Hook-Info-Ignore smb<%s> un-resolved.", old_symbol_name);
        continue;
      }
    }
    symbol_addr = (Elf_Addr)li->baseptr + (Elf_Addr)sym->st_value +
                  (Elf_Addr)sym_hdr->sh_offset;
    LOG(LOG_INFO,
        "Hook-Info-NeedHook oldsmb<%s> oldaddr<%p> oldsize<%u> newsmb<%s> "
        "newaddr<%p> newsize<%u> baseptr<%p> stv<%p> shoff<%p>.",
        old_symbol_name, old_symbol_addr, old_symbol_size, symbol->name,
        symbol_addr, symbol->size, li->baseptr, sym->st_value,
        sym_hdr->sh_offset);
    if (li->rephdr.hookfuns_num >= LNK_MAX_HOOK_FUNC_COUNT) {
      LOG(LOG_DEBUG, "Hook-Info-Ignore smb<%s> hookfunum<%u> exceed MAX<%u>.",
          old_symbol_name, li->rephdr.hookfuns_num, LNK_MAX_HOOK_FUNC_COUNT);
      continue;
    }
    name_len = strlen(symbol->name) + 1;
    old_name_len = strlen(old_symbol_name) + 1;
    hf = &(li->rephdr.hookfuns[li->rephdr.hookfuns_num]);
    strncpy(hf->newname, symbol->name,
            (name_len > LNK_MAX_NAME_LEN) ? LNK_MAX_NAME_LEN : name_len);
    strncpy(
        hf->oldname, old_symbol_name,
        (old_name_len > LNK_MAX_NAME_LEN) ? LNK_MAX_NAME_LEN : old_name_len);
    ...
    /*
    LOG(LOG_INFO, "Hook-Info-Hook newsmb<%s> oldsmb<%s> idx<%u>.", hf->newname,
    hf->oldname, hf->idx);
    */
  }
  return rc;
...
static int linkable_elf_build_rela_info(struct linkable_elf_internals *li) {
  ...
  if (!li || !li->ei.sechdrs || !li->ei.sechdr_num) return -1;
  ...
  li->relahdr_infos = (struct linkable_elf_rela_info *)malloc(
      relahdr_num * sizeof(struct linkable_elf_rela_info));
  if (!li->relahdr_infos) {
    LOG(LOG_ERR, "malloc error: size %d, %s",
        relahdr_num * sizeof(struct linkable_elf_rela_info), strerror(errno));
    return -1;
  }
  memset(li->relahdr_infos, 0,
         relahdr_num * sizeof(struct linkable_elf_rela_info));

  idx = 0;
  for (secidx = 0, idx = 0; (secidx < li->ei.sechdr_num) && (idx < relahdr_num);
       secidx++) {
    if (SHT_RELA == sechdrs[secidx].sh_type) {
      ...
      li->relahdr_infos[idx].sechdr_str_idx =
          sechdrs[sechdrs[secidx].sh_link].sh_link;
      idx++;
      const char *name = &li->ei.strsectbl[sechdrs[secidx].sh_name];
      if (name) {
        LOG(LOG_DEBUG,
            "Find relocation section %s offset: %p "
            "[idx:%u,type:%u,info/dstidx:%u]",
            name, sechdrs[secidx].sh_offset, secidx, sechdrs[secidx].sh_type,
            sechdrs[secidx].sh_info);
      } else {
        LOG(LOG_DEBUG,
            "Find relocation section %s offset: %p "
            "[idx:%u,type:%u,info/dstidx:%u]",
            "N/A", sechdrs[secidx].sh_offset, secidx, sechdrs[secidx].sh_type,
            sechdrs[secidx].sh_info);
      }
    }
    if( (SHT_REL == sechdrs[secidx].sh_type) 
...
static int linkable_elf_build_rela_tobuf(struct linkable_elf_internals *li,
                                         const char *pat_symbol) {
  int rc = 0;
  size_t idx = 0;
  Elf_Shdr *sechdrs = NULL;
  ...
  if (!li || !li->ei.sechdrs || !li->ei.sechdr_num || !li->relahdr_infos ||
      !li->relahdr_info_num)
    return -1;

  LOG(LOG_DEBUG, "Relocatable sections total num %d, start relo...",
      li->relahdr_info_num);
  sechdrs = (Elf_Shdr *)li->ei.sechdrs;
  for (idx = 0; idx < li->relahdr_info_num; idx++) {
    relahdr_info = &(li->relahdr_infos[idx]);
    const char *name =
        &li->ei.strsectbl[sechdrs[relahdr_info->sechdr_idx].sh_name];
    if (name) {
      LOG(LOG_DEBUG,
          "Processing %d relocatable sections %s offset: %p "
          "[idx:%u,type:%u,info/dstidx:%u]",
          idx, name, sechdrs[relahdr_info->sechdr_idx].sh_offset,
          relahdr_info->sechdr_idx, sechdrs[relahdr_info->sechdr_idx].sh_type,
          sechdrs[relahdr_info->sechdr_idx].sh_info);
    } else {
      LOG(LOG_DEBUG,
          "Processing %d relocatable sections %s offset: %p "
          "[idx:%u,type:%u,info/dstidx:%u]",
          idx, "N/A", sechdrs[relahdr_info->sechdr_idx].sh_offset,
          relahdr_info->sechdr_idx, sechdrs[relahdr_info->sechdr_idx].sh_type,
          sechdrs[relahdr_info->sechdr_idx].sh_info);
    }
  ...
  if (failnum) {
    LOG(LOG_ERR, "Relocatable sections process %u/%u failed.", failnum,
        li->relahdr_info_num);
    return -1;
  }
...
static int linkable_elf_build_repfuns(struct linkable_elf_internals *li,
                                      const char *pat_symbol) {
  int rc = 0;
  size_t idx = 0;
  Elf_Sym *sym = NULL;
  ...
  for (idx = 0; idx < li->ei.symbols_num; idx++) {
    ...
    sym = &(symbol->sym);
    LOG(LOG_DEBUG,
        "Rep-Fun-Find smb<%s> add<%p> size<%u> shndx<%u> type<%u> bind<%u>.",
        symbol->name, symbol->address, symbol->size, sym->st_shndx,
        ELF_ST_TYPE(sym->st_info), ELF_ST_BIND(sym->st_info));
    if (sym->st_shndx == SHN_UNDEF) {
    ...
    if (!sym->st_size ||
        ...
        !sym->st_name) {
      LOG(LOG_DEBUG,
          "Rep-Fun-Ignore smb<%s> nameidx<%u> value<%p> hdroff<%p> size<%u>, "
          "one of them is NULL.",
          symbol->name, sym->st_name, sym->st_value, sym_hdr->sh_offset,
          sym->st_size);
      continue;
    ...
    if (strcmp(PAT_ACT_BEFORE_FUN_NAME, symbol->name) == 0) {
      ...
      LOG(LOG_INFO,
          "Rep-Fun-Find-Fun smb<%s> addr<%p> ize<%u> baseptr<%p> stv<%p> "
          "shoff<%p>.",
          symbol->name, symbol_addr, symbol->size, li->baseptr, sym->st_value,
          sym_hdr->sh_offset);
      continue;
    }
    ...
    shtype = LNK_SH_TYPE_FUN;
    if ((old_symbol_addr = linkable_elf_rsv_symbol_repable(
             li, symbol->name, &old_symbol_size, shtype)) <= 0) {
      if ((old_symbol_addr = linkable_elf_pat_repable(
               symbol->name, pat_symbol, &old_symbol_size, 1)) <= 0) {
    ...
    symbol_addr = (Elf_Addr)li->baseptr + (Elf_Addr)sym->st_value +
                  (Elf_Addr)sym_hdr->sh_offset;
    LOG(LOG_INFO,
        "Rep-Fun-NeedRep smb<%s> oldaddr<%p> oldsize<%u> newaddr<%p> "
        "newsize<%u> baseptr<%p> stv<%p> shoff<%p>.",
        symbol->name, old_symbol_addr, old_symbol_size, symbol_addr,
        symbol->size, li->baseptr, sym->st_value, sym_hdr->sh_offset);
    if (li->rephdr.repfuns_num >= LNK_MAX_REP_FUNC_COUNT) {
      LOG(LOG_ERR, "Rep-Fun-Ignore smb<%s> repfunum<%u> exceed MAX<%u>.",
          symbol->name, li->rephdr.repfuns_num, LNK_MAX_REP_FUNC_COUNT);
      continue;
    }
...
struct linkable_elf_internals *linkable_elf_obj_create(
  ...
  do {
    ...
    // LOG(LOG_ERR, "**********Set baseptr to objprt %p, for TEST
    // ONLY!!!**********", li->baseptr);
    //#endif
    ...
    if ((rc = linkable_elf_build_rela_tobuf(li, pat_symbol)) < 0) {
      LOG(LOG_ERR, "The Elf-linkable %s, has some symbol(s) can't be resolved.",
          filename);
      rc = -1;
      break;
    }
    ...
    if ((rc = linkable_elf_build_repfuns(li, pat_symbol)) < 0) {
      LOG(LOG_ERR, "The Elf-linkable %s, build replace funtion list error.",
          filename);
      rc = -1;
      break;
    }
...
/*
static int linkable_elf_load_obj_to_pid(pid_t pid, const char *filename, void *
baseptr, void * base_rephdr_ptr, void * bssptr,  long bsslen, const char
*pat_symbol)
{
    int rc = 0;
        struct linkable_elf_internals *li = NULL;
    li = linkable_elf_obj_create(pid, filename, baseptr, base_rephdr_ptr,
bssptr, bsslen, pat_symbol); if(li){ LOG(LOG_INFO, "Load obj to pid done.");
...
static Elf_Addr linkable_elf_pat_repable(const char *symbolname,
  ...
  // LOG(LOG_DEBUG, "enter func linkable_elf_pat_repable!(%s)",
  // pat_symbol_file);
  ...
  while (NULL_PTR != fgets(symbol_para, 256, fp)) {
    ...
    LOG(LOG_DEBUG, "symbol_para (%s). pat_func_name (%s)", symbolname,
        pat_func_name);
    ...
    if (0 == (strcmp(symbolname, pat_func_name))) {
      ...
      LOG(LOG_DEBUG, "find patch func. name (%s), addr (%p), size (%u)",
          pat_func_name, tptr, *outsymbolsize);
