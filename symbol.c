...
void* symbol_elf_load_section_strings(struct symbol_elf_internals* ei, Elf_Shdr* strh, size_t* outtotalsize)
{
    char* strsymtbl = NULL;
...
void* symbol_elf_load_section_tables(struct symbol_elf_internals* ei, Elf_Shdr* sechdr, size_t* outnum)
{
    void* r_tabs = NULL;
    int rc = 0;
    ...
    {
        LOG(LOG_ERR, "Read section tables error entsize<%d> totalsize<%d>", sechdr->sh_entsize, sechdr->sh_size);
        return NULL;
    }
    do
    {
        ...
        if (!r_tabs)
        {
            LOG(LOG_ERR, "malloc error: size %d, %s", sechdr->sh_size, strerror(errno));
            rc = -1;
            break;
        }
...
static int symbol_elf_load_sym_table(struct symbol_elf_internals* ei, Elf_Shdr* symh, Elf_Shdr* strh)
{
    char* strsymtbl = NULL;
    while (ei && symh && strh)
    {
        ...
        if (symh->sh_entsize > 0 && symh->sh_size > 0)
        {
            ...
            if (!syms)
            {
                LOG(LOG_ERR, "malloc error: size %d, %s", symh->sh_size, strerror(errno));
                break;
            }
            ...
            /* LOG(LOG_DEBUG, "Symbol-sechdr size<%d>, entsize<%d>, offset<%d>", symh->sh_size, symh->sh_entsize, symh->sh_offset); */
            ...
            for (idx = 1; idx < sym_num; ++idx)
            {
                ...
                /*
                LOG(LOG_DEBUG, "Symbol-info idx<%d> info<%d>, nameidx<%d> symb<%p>", idx, syms[idx].st_info, syms[idx].st_name, &syms[idx]);
                */
                ...
                if (name)
                {
                    ...
                    LOG(LOG_DEBUG,"Symbol %u is %s at %p type %d size %u",
                                  idx, name, (void *)syms[idx].st_value, symtype, syms[idx].st_size);
                     */
                    name2 = strdup(name);
                    if (!name2)
                    {
...
static int symbol_elf_load_section_headers_and_symtabs(struct symbol_elf_internals* ei)
{
    ...
    /* LOG(LOG_DEBUG, "Reading section header offset at %u", (size_t)ei->sechdr_offset); */
    ...
    if (!ei->sechdrs_types)
    {
        LOG(LOG_ERR, "malloc error: size %d, %s", sizeof(size_t) * ei->sechdr_num, strerror(errno));
        return -1;
    }
    ...
    for (idx = 0; idx < ei->sechdr_num; ++idx)
    {
        ...
        if (name)
        {
        	LOG(LOG_DEBUG, "Section name: %s Addr: %p Len: %u Idx: %u", name, (void *)sechdrs[idx].sh_offset, sechdrs[idx].sh_size, idx);
        }
        else
        {
        	LOG(LOG_DEBUG, "Section name: %s Addr: %p Len: %u Idx: %u", "N/A", (void *)sechdrs[idx].sh_offset, sechdrs[idx].sh_size, idx);
        }
        */
        ...
        switch (sechdrs[idx].sh_type)
        {
            ...
            case SHT_DYNSYM:
                symtab = idx;
                /*
                LOG(LOG_DEBUG, "Symbol table offset: %u size: %u entsize: %u entries: %u",
                              sechdrs[idx].sh_offset, sechdrs[idx].sh_size, sechdrs[idx].sh_entsize, (sechdrs[idx].sh_entsize > 0 ? sechdrs[idx].sh_size / sechdrs[idx].sh_entsize : 0));
                */
                break;
            case SHT_STRTAB:
                if (idx != ei->secnametbl_idx)
                {
                    ...
                    if (symtab >= 0 && symbol_elf_load_sym_table(ei, &sechdrs[symtab], &sechdrs[strtab]) < 0)
                    {
    ...
    if (ei->type == ET_REL)
    {
        LOG(LOG_DEBUG, "Find linkable sections bss_idx(%u) text_idx(%u) data_idx(%u) rodata_idx(%u) ",
            ei->sechdr_idx_bss, ei->sechdr_idx_text, ei->sechdr_idx_data, ei->sechdr_idx_rodata);
    }
    return 0;
}
...
static int symbol_elf_load_program_headers(struct symbol_elf_internals* ei)
{
    ...
    for (idx = 0; idx < ei->proghdr_num; ++idx)
    {
        ...
        /*
        LOG(LOG_DEBUG,"Prog-header %u: Type: %d VAddr: %p PAddr: %p FileSz: %u MemSz: %u",
        			idx, proghdrs[idx].p_type, (void *)proghdrs[idx].p_vaddr, (void *)proghdrs[idx].p_paddr,
        ...
        */
        if (proghdrs[idx].p_type == PT_INTERP)
        {
            ...
            /* LOG(LOG_DEBUG, "Found %s at V-Addr %p", ei->interp.name, ei->interp.ph_addr); */
        }
            ...
            if (cnt_LOAD == 0 ) {
                if (ei->type == ET_EXEC) {
                    ...
                    if (proghdrs[idx].p_offset != 0 || proghdrs[idx].p_vaddr != 0) {
                        LOG(LOG_ERR, "ELF file is not support: ET_DYN with none zero PT_LOAD[0]");
                        rc = -1;
                        break;
                    }
                }
...
void symbol_elf_ei_destory(struct symbol_elf_internals* ei, int needfreesymbol)
{
    if (!ei)
    {
        return;
...
static struct symbol_elf_sym* symbol_elf_load_file(const char* filename,
        size_t* symbols_num,
                                    ...
                                    const char* appname)
{
    ...
    do
    {
        ...
        if (!pm->addr_valid)
        {
            LOG(LOG_DEBUG, "[%s:%d] Strtoul error(%s) in parsing %s", strerror(err), token);
        }
        token = strtok_r(NULL, " ", &save);
        if (!token) { break; }
        ...
        if (!pm->pathname)
        {
            LOG(LOG_ERR, "malloc error: size %d, %s", pm->pathname_sz + 1, strerror(errno));
            pm->pathname = NULL;
            pm->pathname_sz = 0;
            break;
        }
        ...
        if ((token = strchr(save, '/')))
        {
            ...
            else
            {
                ...
                {
                    int err = errno;
                    LOG(LOG_DEBUG, "Unable to stat file %s. Error: %s", pm->pathname, strerror(err));
                }
            }
        }
...
static struct symbol_ld_procmaps* symbol_ld_load_maps(pid_t pid, size_t* num, uintptr_t* base_adjust)
{
    ...
    do
    {
        ...
        if (!maps)
        {
            LOG(LOG_ERR, "malloc error: size %d, %s", mapmax * sizeof(*maps), strerror(errno));
            break;
        }
        ...
        while (fgets(buf, bufsz, ff))
        {
            ...
            if (need_adjust == 1 && cnt_LOAD == 0) {
                if (pm->filetype == PROCMAPS_FILETYPE_EXE) {
                    if (!pm->addr_valid) {
                        LOG(LOG_ERR, "/proc/%d/maps addr invalid for %s", pid, pm->pathname);
                        break;
                    }
...
static int symbol_ld_find_library(const struct symbol_ld_procmaps* maps, const size_t mapnum,
                                  const char* libpath, int inode_match,
                                  struct symbol_ld_library* lib)
{
    ...
    else
    {
        ...
        if (inode_match)
        {
            ...
            {
                int err = errno;
                LOG(LOG_ERR, "Unable to get inode for %s. Error: %s", libpath, strerror(err));
                return -1;
            }
...
static uintptr_t symbol_ld_find_sym_a(const struct symbol_ld_library* lib, int intype, const char* symbol, size_t* sz)
{
    uintptr_t ptr = 0;
    if (lib && symbol && lib->pathname)
    {
        size_t syms_num = 0;
        struct symbol_elf_sym* syms = symbol_elf_load_file(lib->pathname, &syms_num, NULL, NULL, NULL, NULL);
        if (syms && syms_num > 0)
        {
            size_t idx = 0;
            ...
            for (idx = 0; idx < syms_num; ++idx)
            {
                ...
                {
                    LOG(LOG_DEBUG, "Found %s in symbol list at %u with address offset %p in %s",
                ...
                {
                    /*
                    LOG(LOG_DEBUG, "Found symbol but not equal [%s != %s] lib %s ", syms[idx].name, symbol, lib->pathname);
                    */
                }
            }
...
uintptr_t symbol_ld_find_sym_repable(struct symbol_elf_pid* hp, int intype, const char* symbol, size_t* sz)
{
    uintptr_t ptr = 0;
    ...
    if (hp && hp->ld_maps_num && hp->ld_maps)
    {
        ...
        for (ddx = ELF_LIB_C; ddx < ELF_LIB_MAX; ddx++)
        {
            ...
            if (ptr)
            {
                LOG(LOG_DEBUG, "Find symbol<%s> ptr:%p in default lib:<%s>", symbol, ptr, lib->pathname);
                return ptr;
            }
        }
    }
    ...
    if (hp && hp->ld_maps_num && hp->ld_maps)
    {
        ...
        for (idx = 0; idx < hp->ld_maps_num; idx++)
        {
            ...
            if (ptr)
            {
                LOG(LOG_DEBUG, "Find symbol<%s> ptr:%p in lib:<%s>", symbol, ptr, lib->pathname);
                if (lib->pathname)
                { free(lib->pathname); }
                lib->pathname = NULL;
                break;
...
uintptr_t symbol_ld_find_sym(struct symbol_elf_pid* hp, int intype, const char* symbol, size_t* sz)
{
    uintptr_t ptr = 0;
    struct symbol_ld_library lib_s;
    struct symbol_ld_library* lib = &lib_s;
...
{
    uintptr_t ptr = symbol_ld_find_sym_repable(hp, SYMBOL_IS_FUNCTION, symbol, sz);
    return ptr;
}
...
uintptr_t symbol_ld_find_func(struct symbol_elf_pid* hp, const char* symbol, size_t* sz)
{
    uintptr_t ptr = symbol_ld_find_sym(hp, SYMBOL_IS_FUNCTION, symbol, sz);
    return ptr;
}

uintptr_t symbol_ld_find_global(struct symbol_elf_pid* hp, const char* symbol, size_t* sz)
{
...
#define LD_PROCMAPS_FIND_LIB(name,flag,index,retval) \
    do { \
        ...
        if (symbol_ld_find_library(hp->ld_maps, hp->ld_maps_num, \
                                   name, flag, &hp->libs[index]) < 0) { \
...
#define LD_LIB_FIND_FN_ADDR(fn,outfn,index) \
    do { \
        if (outfn) break; \
        outfn = symbol_ld_find_sym_a(&hp->libs[ELF_##index],SYMBOL_IS_FUNCTION,fn,&symbsize); \
        if (outfn != 0) { \
    ...
    {
        LOG(LOG_ERR, "Some memory allocation routines are unavailable. Cannot proceed.");
        return -1;
    }
    ...
    if (!hp->fn_dlopen || !hp->fn_dlsym)
    {
        LOG(LOG_ERR, "Dynamic Library loading routines were not found. Cannot proceed.");
        return -1;
    }
    ...
    {
        LOG(LOG_INFO, "Pthread's symbol not found. Will disable pthread usage in injection.");
    }
...
struct symbol_elf_pid* symbol_pid_create_inner(pid_t pid, int symelang, int needlibc)
{
    int rc  = 0;
    int idx = 0;
    uintptr_t ptr = 0;
    ...
    do
    {
        ...
        hp->elang = symelang;
        hp->exe_symbols = symbol_elf_load_file(filename, &hp->exe_symbols_num, &hp->exe_entry_point, &hp->exe_base_adjust, &hp->exe_interp, &hp->is64);
        if (!hp->exe_symbols)
        {
        ...
        hp->ld_maps = symbol_ld_load_maps(hp->pid, &hp->ld_maps_num, &hp->exe_base_adjust);
        if (!hp->ld_maps)
        {
        ...
        {
            qsort(hp->exe_symbols, hp->exe_symbols_num, sizeof(*hp->exe_symbols), symbol_elf_symbol_cmpqsort);
...
static uintptr_t symbol_pid_find_sym_a(struct symbol_elf_pid* hp, int intype, const char* symbol, size_t* sz)
{
    uintptr_t ptr = 0;
    size_t idx = 0;
...
uintptr_t symbol_pid_find_func(struct symbol_elf_pid* hp, const char* symbol, size_t* sz)
{
    uintptr_t ptr = symbol_pid_find_sym_a(hp, SYMBOL_IS_FUNCTION, symbol, sz);
    return ptr;
}

uintptr_t symbol_pid_find_global(struct symbol_elf_pid* hp, const char* symbol, size_t* sz)
{
