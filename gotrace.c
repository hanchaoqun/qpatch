...
struct hashmap* user_hashmap_init(int size) {
  return hashmap_new(sizeof(struct user_hashmap), size, 0, 0, user_hash,
                     user_compare, NULL, NULL);
}

void* user_hashmap_get(struct hashmap* map, uint64_t key) {
  const struct user_hashmap* user =
      hashmap_get(map, &(struct user_hashmap){.key = key});
  if (user == NULL) {
    return NULL;
  }
  return user->data;
...
void* user_hashmap_set(struct hashmap* map, uint64_t key, void* data) {
  const struct user_hashmap* user =
      hashmap_set(map, &(struct user_hashmap){.key = key, .data = data});
  return (void*)user;
}
...
// Go 1.16
//  <+0> : mov %fs:0xfffffffffffffff8,%rcx  => 0x64 0x48 0x8b 0x0c 0x25 0xf8
//  0xff 0xff 0xff
...
struct tag_gotrace_go_pre {
  ...
  int bpoff;
    ...
    /* go 1.16 */
    {{0x64, 0x48, 0x8b, 0x0c, 0x25, 0xf8, 0xff, 0xff, 0xff, 0x48, 0x3b, 0x61,
      0x10},
     {0x64, 0x48, 0x8b, 0x0c, 0x25, 0xf8, 0xff, 0xff, 0xff, 0x90, 0x90, 0x90,
      0xcc},
     13,
     13},
    /* go 1.17 */
    {{0x49, 0x3b, 0x66, 0x10}, {0x90, 0x90, 0x90, 0xcc}, 4, 4},
    /* c/c++ */
    {{0x55, 0x48, 0x89, 0xe5}, {0xcc, 0x48, 0x89, 0xe5}, 4, 1},
};
...
void gotrace_oncallin_116(struct ptrace_pid* pp, struct user* oregs,
                          struct symbol_elf_sym* sym);
void gotrace_oncallin_117(struct ptrace_pid* pp, struct user* oregs,
                          struct symbol_elf_sym* sym);
void gotrace_oncallin_cpp(struct ptrace_pid* pp, struct user* oregs,
                          struct symbol_elf_sym* sym);
...
struct symbol_elf_sym* gotrace_getsymbyaddr(uintptr_t addr) {
  ...
  sym =
      (struct symbol_elf_sym*)user_hashmap_get(g_add2sym_maps, (uint64_t)addr);
  return sym;
}

struct symbol_elf_sym* gotrace_getelfsym(struct ptrace_pid* pp,
                                         const char* name) {
  struct symbol_elf_sym* sym = NULL;
...
int gotrace_set_breakpoint_all(struct ptrace_pid* pp) {
  ...
  for (int idx = 0; idx < pp->hp->exe_symbols_num; ++idx) {
    sym = &(pp->hp->exe_symbols[idx]);
    if (SYMBOL_IS_FUNCTION != sym->type || sym->address == 0 ||
        gotrace_filter(sym) == -1) {
      continue;
    }
    symtmp = gotrace_getsymbyaddr(sym->address);
    if (symtmp != NULL) {
      LOG(LOG_ERR, "sym address <%p> with name <%s> already insterted by <%s>!",
          sym->address, sym->name, symtmp->name);
      continue;
    }
    sym->cppname = NULL;
...
int gotrace_set_breakpoint(struct ptrace_pid* pp, struct symbol_elf_sym* sym) {
  ...
  do {
    if ((rc = ptrace_pid_readarray(pid, sym->address, tmpopcode,
                                   GOTRACE_GO_PRE_OPCODE_MAX)) < 0) {
      LOG(LOG_ERR, "Process <%s> can't read fun from(%p) len(%u).", sym->name,
          sym->address, GOTRACE_GO_PRE_OPCODE_MAX);
      break;
    }
    for (idx = 0; idx < GOTRACE_GO_PRE_CNT; idx++) {
      if (memcmp(tmpopcode, gotrace_go_pre[idx].opcodes,
                 gotrace_go_pre[idx].codesize) == 0) {
        sym->gopreidx = idx + 1;
        break;
      }
    }
    if (sym->gopreidx == 0) {
      LOG(LOG_INFO, "Process <%s>(%p) go pre code is not expected!", sym->name,
          sym->address);
      break;
    }
    idx = sym->gopreidx - 1;
    if ((rc = ptrace_pid_writearray(pid, sym->address,
                                    (unsigned char*)gotrace_go_pre[idx].bpcodes,
                                    gotrace_go_pre[idx].codesize)) < 0) {
      LOG(LOG_ERR, "Process <%s> can't write to(%p) len(%u).", sym->name,
          sym->address, gotrace_go_pre[idx].codesize);
      break;
    }
    LOG(LOG_INFO, "set breakpoint <%s>(%p) gopreidx<%d> ok!", sym->name,
        sym->address, sym->gopreidx);
    sym->setbp = 1;
    rc = 0;
  } while (0);
...
int gotrace_unset_breakpoint(struct ptrace_pid* pp,
                             struct symbol_elf_sym* sym) {
  int rc = -1;
  pid_t pid = pp->pid;
  unsigned char tmpopcode[GOTRACE_GO_PRE_OPCODE_MAX];
  ...
  do {
    ...
    if (memcmp(tmpopcode, gotrace_go_pre[idx].bpcodes,
               gotrace_go_pre[idx].codesize) != 0) {
      break;
    }
    if ((rc = ptrace_pid_writearray(pid, sym->address,
                                    (unsigned char*)gotrace_go_pre[idx].opcodes,
                                    gotrace_go_pre[idx].codesize)) < 0) {
      LOG(LOG_ERR, "Process <%s> can't write to(%p) len(%u).", sym->name,
          sym->address, gotrace_go_pre[idx].codesize);
      break;
    }
    LOG(LOG_INFO, "unset breakpoint <%s>(%p) gopreidx<%d> ok!", sym->name,
        sym->address, sym->gopreidx);
    sym->setbp = 0;
    rc = 0;
  } while (0);
...
int gotrace_on_breakpoint(struct ptrace_pid* pp, struct user* oregs,
                          struct symbol_elf_sym* sym) {
  int rc = -1;
...
void gotrace_oncallin_116(struct ptrace_pid* pp, struct user* oregs,
  ...
  printf("mid[%08ld] goid[%06ld]: [%p] %s <-( ", (uintptr_t)pid, goid,
         (void*)sym->address, sym->name);
  ptrace_pid_readarray(pid, rsp + 8, (unsigned char*)args,
                       sizeof(uintptr_t) * GOTRACE_GO_SHOW_ARG_NUM);
...
void gotrace_oncallin_117(struct ptrace_pid* pp, struct user* oregs,
  ...
  printf(
      ...
      "0x%llx, ",
      PTRACE_REG_AX(*oregs), PTRACE_REG_BX(*oregs), PTRACE_REG_CX(*oregs),
      PTRACE_REG_DI(*oregs), PTRACE_REG_SI(*oregs), PTRACE_REG_R8(*oregs),
      PTRACE_REG_R9(*oregs), PTRACE_REG_R10(*oregs), PTRACE_REG_R11(*oregs));
  printf("... )\n");
...
void gotrace_oncallin_cpp(struct ptrace_pid* pp, struct user* oregs,
  ...
  ptrace_pid_readarray(pid, rsp + 8, (unsigned char*)args,
                       sizeof(uintptr_t) * GOTRACE_C_SHOW_ARG_NUM);
  printf("tid[%08ld]: [%p] %s <-( ", (uintptr_t)pid, (void*)sym->address,
         (sym->cppname == NULL) ? sym->name : sym->cppname);
  printf("0x%llx, 0x%llx, 0x%llx, 0x%llx, ", PTRACE_REG_CX(*oregs),
         PTRACE_REG_DX(*oregs), PTRACE_REG_R8(*oregs), PTRACE_REG_R9(*oregs));
