...
static UINT32 LOG_TextAdapt (UINT32 enLevel,
                                 UINT32 ulModule,
                                 UINT32 enLogType,
                                 CHAR* pMessage,
                                 ...)
{
    ...
    time(&now);
    tnow = localtime(&now);
    lOffset = snprintf(logText, MAX_TRACE_LEN - 1, "%04d_%02d_%02d %02d:%02d:%02d ",
...
void* qpatch_open_room(char* data, size_t len)
{
    ...
    if (g_qpatch_room_ptr)
    {
        ...
        {
            LOG(LOG_ERR, "mmap(%p) can't resize len from(%u) to(%u)", g_qpatch_room_ptr, g_qpatch_room_size, A->para1);
            return g_qpatch_room_ptr;
        }
    }
    do
    {
        baseptr = mmap(0, A->para1, PROT_EXEC | PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, 0, 0);
...
int qpatch_close_room(char* data, size_t len)
{
    ...
    do
    {
        ...
        {
            rc = -1;
            LOG(LOG_ERR, "munmap(%p) len(%u) error: %s", g_qpatch_room_ptr, g_qpatch_room_size, strerror(errno));
            break;
        }
        g_qpatch_room_ptr = 0;
