...
#define PTRACE_ASM_SET_REGS(pid, fn, iregs, rc)               \
  do {                                                        \
    ...
    if ((rc = ptrace_pid_setregs(pid, &iregs)) < 0) break;    \
  } while (0)
...
#define PTRACE_ASM_CALL_FUNC(pid, fn, iregs, rc)                           \
  do {                                                                     \
    ...
    if ((rc = ptrace_pid_cont(pid)) < 0) break;                            \
    LOG(LOG_DEBUG, "Waiting...");                                          \
    if ((rc = ptrace_pid_wait(pid)) < 0) break;                            \
    LOG(LOG_DEBUG, "Getting registers.");                                  \
    if ((rc = ptrace_pid_getregs(pid, &iregs)) < 0) break;                 \
    LOG(LOG_DEBUG, "Registers: IP:%p, SP:%p, AX:%p", PTRACE_REG_IP(iregs), \
        PTRACE_REG_SP(iregs), PTRACE_REG_AX(iregs));                       \
...
#define PTRACE_ASM_MEMCPY2STACK(pid, iregs, mem, memsize, retmem, rc)          \
  do {                                                                         \
    ...
    if ((rc = ptrace_pid_writearray(pid, PTRACE_REG_SP(iregs),                 \
                                    (unsigned char*)mem, (size_t)(memsize))) < \
        0)                                                                     \
      break;                                                                   \
    retmem = PTRACE_REG_SP(iregs);                                             \
    LOG(LOG_DEBUG, "Copy mem to SP[%p] V[%p] Size[%d] done.",                  \
        PTRACE_REG_SP(iregs), mem, memsize);                                   \
  } while (0)
...
#define PTRACE_ASM_SET_BREAKPOINT(pid, iregs, rc)                             \
  do {                                                                        \
    ...
    LOG(LOG_DEBUG, "Alignment for gcc ABI : from SP[%p]",                     \
        PTRACE_REG_SP(iregs));                                                \
    ...
    LOG(LOG_DEBUG, "Alignment for gcc ABI : to   SP[%p]",                     \
        PTRACE_REG_SP(iregs));                                                \
    LOG(LOG_DEBUG, "Setting breakpoint to stack...");                         \
    if ((rc = ptrace_pid_writelong(pid, PTRACE_REG_SP(iregs), nullcode)) < 0) \
      break;                                                                  \
    LOG(LOG_DEBUG, "Set breakpoint SP[%p] V[%p] done.", PTRACE_REG_SP(iregs), \
        nullcode);                                                            \
  } while (0)
...
#define PTRACE_ASM_RECOVER_REGS(pid, iregs, oregs, rc)                       \
  do {                                                                       \
    ...
    if ((rc = ptrace_pid_setregs(pid, &oregs)) < 0) {                        \
      LOG(LOG_ERR,                                                           \
          "PID %d will be unstable, set original registers error : %s", pid, \
          strerror(errno));                                                  \
      break;                                                                 \
    }                                                                        \
  } while (0)
...
#define PTRACE_ASM_PASS_ARGS2SYSCALL7(pid, A, SYSCALLNO, ARG1, ARG2, ARG3, \
                                      ARG4, ARG5, ARG6, ARG7, rc)          \
  do {                                                                     \
    PTRACE_REG_SP(iregs) = PTRACE_REG_SP(iregs) - 16;                      \
    if ((rc = ptrace_pid_writelong(pid, PTRACE_REG_SP(iregs) + 8, ARG7)) < \
        0) {                                                               \
      PTRACE_REG_SP(iregs) = PTRACE_REG_SP(iregs) + 16;                    \
      break;                                                               \
    }                                                                      \
    ...
    LOG(LOG_DEBUG,                                                         \
        "ARGS2SYSCALL[%p]: ARG1:%p, ARG2:%p, ARG3:%p, ARG4:%p, ARG5:%p, "  \
        "ARG6:%p, ARG7:%p",                                                \
        SYSCALLNO, ARG1, ARG2, ARG3, ARG4, ARG5, ARG6, ARG7);              \
    rc = 0;                                                                \
    pid = pid;                                                             \
  } while (0)
...
#define PTRACE_ASM_RUN_SYSCALL7(pid, iregs, rc)                            \
  do {                                                                     \
    ...
    LOG(LOG_DEBUG, "Registers: IP:%p, SP:%p, AX:%p", PTRACE_REG_IP(iregs), \
        PTRACE_REG_SP(iregs), PTRACE_REG_AX(iregs));                       \
    if (!((int64_t)PTRACE_REG_AX(iregs) >= -4095 &&                        \
          (int64_t)PTRACE_REG_AX(iregs) <= -1)) {                          \
      LOG(LOG_DEBUG, "Syscall function call finished, return %p.",         \
          PTRACE_REG_AX(iregs));                                           \
      break;                                                               \
    }                                                                      \
    rc = -1;                                                               \
    LOG(LOG_DEBUG, "Syscall return error : %p, try wait again...",         \
        PTRACE_REG_AX(iregs));                                             \
  } while (0)

#define PTRACE_ASM_PASS_ARGS2SYSCALL6(pid, A, SYSCALLNO, ARG1, ARG2, ARG3, \
                                      ARG4, ARG5, ARG6, rc)                \
  do {                                                                     \
    A.regs.r9 = ARG6;                                                      \
    A.regs.r8 = ARG5;                                                      \
    ...
    LOG(LOG_DEBUG,                                                         \
        "ARGS2SYSCALL[%p]: ARG1:%p, ARG2:%p, ARG3:%p, ARG4:%p, ARG5:%p, "  \
        "ARG6:%p",                                                         \
        SYSCALLNO, ARG1, ARG2, ARG3, ARG4, ARG5, ARG6);                    \
    rc = 0;                                                                \
    pid = pid;                                                             \
  } while (0)
...
extern int ptrace_pid_readlong(pid_t pid, uintptr_t target,
                               uintptr_t* outvalue);
...
extern int ptrace_pid_call_func_noparam(pid_t pid, uintptr_t funcaddr,
                                        uintptr_t* outcallret);
extern int ptrace_pp_read_data(struct ptrace_pid* pp, uintptr_t target,
                               unsigned char* outdata, size_t outdatalen);
extern int ptrace_pp_write_data(struct ptrace_pid* pp, uintptr_t target,
                                const unsigned char* indata, size_t indatalen);
extern int ptrace_pp_call_library(struct ptrace_pid* pp, uintptr_t indlladdr,
                                  const char* callsymbol,
                                  const unsigned char* paradata,
                                  size_t paradatalen, uintptr_t* outcallret,
                                  int needattach);
extern int ptrace_pid_inject_libc(pid_t pid, int elang, const char* libcname,
                                  const size_t libcsize);
extern int ptrace_pp_inject_library(struct ptrace_pid* pp, const char* dll,
                                    const char* callsymbol,
                                    const unsigned char* paradata,
                                    size_t paradatalen, uintptr_t* outdlladdr,
                                    uintptr_t* outcallret);
